...
use oxidowl::ontology::{ClassExpression, IRI, Individual, ObjectPropertyExpression, Ontology};
    ...
    ConjunctiveQuery {
        ...
        body_atoms: vec![
            QueryAtom::ClassAtom {
                ...
                class_expression: ClassExpression::class(IRI::new(
                    "http://test.example.org/test#Person",
                )),
            },
            QueryAtom::PropertyAtom {
                subject: QueryVariable::new("x".to_string()),
                property: ObjectPropertyExpression::object_property(IRI::new(
                    "http://test.example.org/test#knows",
                )),
    ...
    ConjunctiveQuery {
        ...
        body_atoms: vec![
            QueryAtom::PropertyAtom {
                subject: QueryVariable::new("x".to_string()),
                property: ObjectPropertyExpression::object_property(IRI::new(
                    "http://test.example.org/test#connected",
                )),
                object: QueryVariable::new("y".to_string()),
            },
            QueryAtom::PropertyAtom {
...
fn test_ml_strategy_selection_e2e() {
    ...
    let engine = AdvancedExecutionEngine::new(ontology_arc.clone(), reasoning_service, config)
    ...
    match result {
        Ok(query_result) => {
            ...
            println!(
                "  Execution time: {:?}",
                query_result.metadata.execution_time
            );
            println!("  Result count: {}", query_result.bindings.len());

            // Verify metadata is populated
            assert!(
                !query_result.metadata.strategy_used.is_empty(),
                "Strategy should be recorded"
            );
            assert!(
                query_result.metadata.execution_time > Duration::from_secs(0),
...
fn test_fallback_to_alternatives() {
    ...
    let engine = AdvancedExecutionEngine::new(ontology.clone(), reasoning_service, config)
        .expect("Failed to create execution engine");
...
fn test_online_learning_triggered() {
    ...
    println!();
    println!(
        "✓ Executed {} queries ({} successful, {} failed)",
    ...
    assert!(
        successful_queries + failed_queries == 32,
...
fn test_concurrent_execution() {
    ...
    let engine = Arc::new(
        AdvancedExecutionEngine::new(ontology.clone(), reasoning_service, config)
            .expect("Failed to create execution engine"),
    );
...
fn test_performance_overhead() {
    ...
    let engine_ml =
        AdvancedExecutionEngine::new(ontology.clone(), reasoning_service.clone(), config_ml)
    ...
    let engine_legacy =
        AdvancedExecutionEngine::new(ontology.clone(), reasoning_service, config_legacy)
    ...
    assert!(
        overhead < 50.0,
        "ML overhead should be reasonable (got {:.2}%)",
        overhead
    );
}
...
fn test_ml_config_variations() {
    ...
    let engine1 =
        AdvancedExecutionEngine::new(ontology.clone(), reasoning_service.clone(), config1);
    ...
    let engine2 =
        AdvancedExecutionEngine::new(ontology.clone(), reasoning_service.clone(), config2);
...
fn test_error_handling_robustness() {
    ...
    let result2 =
        rt.block_on(engine.execute_query(&invalid_query, default_execution_constraints()));
