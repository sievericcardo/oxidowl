...
use oxidowl::ontology::{Class, ClassExpression, IRI, Ontology, ObjectPropertyExpression};
use oxidowl::ontology::axioms::{Axiom, SubClassOfAxiom, ClassAssertionAxiom, ObjectPropertyAssertionAxiom};
...
fn test_el_reasoner_basic() -> Result<()> {
    ...
    if let Some(superclasses) = hierarchy.hierarchy.get(&ClassExpression::Class(dog.clone())) {
...
fn test_rl_reasoner_property_hierarchy() -> Result<()> {
    ...
    // Properties
    let has_parent = ObjectPropertyExpression::ObjectProperty(oxidowl::ontology::ObjectProperty { iri: IRI::new("http://example.org/hasParent") });
    let has_ancestor = ObjectPropertyExpression::ObjectProperty(oxidowl::ontology::ObjectProperty { iri: IRI::new("http://example.org/hasAncestor") });
    
    // hasParent ⊑ hasAncestor
    ontology.add_axiom(Axiom::SubObjectPropertyOf(oxidowl::ontology::axioms::SubObjectPropertyOfAxiom {
        id: 0,
    ...
    assert!(!result.hierarchy.is_empty(), "Hierarchy should contain classes");
    assert!(result.hierarchy.contains_key(&ClassExpression::Class(person)), "Hierarchy should contain Person class");
...
fn test_rl_reasoner_hierarchy_correctness() -> Result<()> {
    ...
    assert!(result.hierarchy.contains_key(&ClassExpression::Class(animal.clone())));
    assert!(result.hierarchy.contains_key(&ClassExpression::Class(mammal.clone())));
    assert!(result.hierarchy.contains_key(&ClassExpression::Class(dog.clone())));
    
    // Verify Dog ⊑ Mammal (transitive closure: Dog should have Mammal as superclass)
    let dog_superclasses = result.hierarchy.get(&ClassExpression::Class(dog.clone())).unwrap();
    assert!(dog_superclasses.contains(&ClassExpression::Class(mammal.clone())), 
            "Dog should be subclass of Mammal");
    
    // Verify transitivity: Dog ⊑ Animal
    assert!(dog_superclasses.contains(&ClassExpression::Class(animal.clone())), 
            "Dog should be subclass of Animal (via transitivity)");
    
    // Verify reflexivity: Dog ⊑ Dog
    assert!(dog_superclasses.contains(&ClassExpression::Class(dog.clone())), 
