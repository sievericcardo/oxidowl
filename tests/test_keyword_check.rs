...
Ontology(<http://example.org/test>
    ...
    match result {
        Err(e) => {
            ...
            assert!(
                err_msg.contains("ObjectSomeValuesFrom"),
                "Error should mention ObjectSomeValuesFrom"
            );
            assert!(
                err_msg.contains("keyword") || err_msg.contains("IRI"),
...
Ontology(<http://example.org/test>
    ...
    assert!(
        result.is_ok(),
        "Prefixed ObjectSomeValuesFrom should be allowed"
    );
}
...
Ontology(<http://example.org/test>
    ...
    assert!(
        result.is_ok(),
        "Bracketed ObjectSomeValuesFrom should be allowed"
    );
}
