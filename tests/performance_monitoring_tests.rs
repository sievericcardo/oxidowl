...
fn test_memory_tracking_integration() {
    ...
    for i in 1..=10 {
        ...
        let _ = tracker.snapshot(cache_size, reasoning_state).expect("Failed to record snapshot");
    }
...
fn test_query_profiling_integration() {
    ...
    assert_eq!(
        stats.total_atoms_evaluated,
        (1..=20).map(|i| 10 * i).sum::<usize>()
    );
    assert_eq!(
        stats.total_joins_performed,
        (1..=20).map(|i| 5 * i).sum::<usize>()
    );
...
fn test_performance_monitor_integration() {
    ...
    for i in 1..=5 {
        let _ = monitor.snapshot_memory(1024 * 1024 * i, 512 * 1024 * i).expect("Failed to record memory snapshot");
        ...
        let _ = monitor.record_query_timing(timing).expect("Failed to record query timing");
    }
...
fn test_performance_monitor_disabled() {
    ...
    let _ = monitor.record_query_timing(timing).expect("Failed to record timing");
...
fn test_memory_snapshot_calculations() {
    ...
    let _ = tracker.snapshot(2 * 1024 * 1024, 3 * 1024 * 1024).expect("Failed to record snapshot");
    ...
    let expected_total =
        snapshot.heap_allocated + snapshot.cache_size + snapshot.reasoning_state_size;
...
fn test_tracker_max_snapshots_limit() {
    ...
    // Take 5 snapshots
    for i in 1..=5 {
        let _ = tracker.snapshot(1024 * i, 512 * i).expect("Failed to record snapshot");
    }
