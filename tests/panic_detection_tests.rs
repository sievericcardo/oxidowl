...
fn test_empty_collection_access_returns_error() {
    ...
    assert!(result.is_none(), "Popping from empty queue should return None");
}
...
fn test_data_structure_edge_cases() {
    ...
    assert!(result.is_ok(), "Reading lock on new ontology should succeed");
    
    let guard = result.unwrap();
    // The signature should exist (though empty) - no unwrap panic
    let sig_result = guard.signature();
    assert!(sig_result.is_ok(), "Getting signature should return Result, not panic");
}
...
fn test_option_unwrap_replacements() {
    ...
    assert!(result.is_none(), "last() on empty vec should be None, not unwrap panic");
    ...
    assert!(error_result.is_err(), "Should return error for empty collection");
    match error_result {
...
fn test_collection_operations_without_panic() {
    ...
    assert!(first_empty.is_none(), "first() on empty vec returns None, not panic");
...
fn test_fingerprint_option_handling() {
    ...
    let result = fingerprint.ok_or_else(|| {
        Error::internal("Fingerprint is None despite check")
    });
    
    assert!(result.is_err(), "Should return error when fingerprint is None");
    match result {
    ...
    let result_some = fingerprint_some.ok_or_else(|| {
        Error::internal("Fingerprint is None")
    });
