...
mod missing_features_tests {
    ...
    fn test_timezone_builtin() {
        ...
        if let Some(builtin) = registry.get_builtin("http://www.w3.org/2003/11/swrlb#timezone") {
            ...
            let result = builtin
                .execute(&[result_var, dt_utc])
                .expect("timezone execution should succeed");
            ...
            match result {
                SWRLValue::Literal(literal) => {
                    ...
                    assert_eq!(
                        literal.datatype,
                        Some(url::Url::parse("http://www.w3.org/2001/XMLSchema#string").unwrap())
                    );
                }
    ...
    fn test_add_year_month_duration_builtin() {
        ...
        if let Some(builtin) =
            registry.get_builtin("http://www.w3.org/2003/11/swrlb#addYearMonthDuration")
        {
            ...
            let duration = SWRLValue::Literal(Literal {
                ...
                datatype: Some(
                    url::Url::parse("http://www.w3.org/2001/XMLSchema#yearMonthDuration").unwrap(),
                ),
                language: None,
            });

            let result = builtin
                .execute(&[result_var, base_date, duration])
                .expect("addYearMonthDuration execution should succeed");
            ...
            match result {
                SWRLValue::Literal(literal) => {
                    ...
                    assert_eq!(
                        literal.datatype,
                        Some(url::Url::parse("http://www.w3.org/2001/XMLSchema#date").unwrap())
                    );
                }
                _ => panic!("Expected literal result"),
            }
    ...
    fn test_add_year_month_durations_builtin() {
        ...
        if let Some(builtin) =
            registry.get_builtin("http://www.w3.org/2003/11/swrlb#addYearMonthDurations")
        {
            ...
            let result = builtin
                .execute(&[result_var, duration1, duration2])
                .expect("addYearMonthDurations execution should succeed");
    ...
    fn test_duration_edge_cases() {
            ...
            registry.get_builtin("http://www.w3.org/2003/11/swrlb#addYearMonthDuration")
        {
            ...
            let result = builtin
                .execute(&[result_var, base_date, zero_duration])
                .expect("zero duration execution should succeed");
