...
async fn test_swrl_execution_method_available() {
    ...
    assert!(
        !swrl_result.fired,
        "Should not fire any rules when there are no SWRL rules"
    );
    assert_eq!(
        swrl_result.inferences.len(),
        0,
        "Should have no inferences when no rules fire"
    );
...
async fn test_swrl_rules_applied_during_reasoning() {
    ...
    assert!(
        swrl_execution_result.is_ok(),
        "SWRL execution should succeed"
    );

    println!(
        "✅ SWRL rules applied during reasoning! Service integrates SWRL functionality correctly"
    );
}
