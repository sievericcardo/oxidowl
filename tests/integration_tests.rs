...
use oxidowl::{
    config::{OWLProfile, OxidowlConfig, ServerConfig},
    error::Error,
    explanation::{ExplanationFormat, ExplanationService, ExplanationType},
    import::resolver::{ImportResolver, ImportResolverConfig},
    ontology::{Class, ClassExpression, Individual, Ontology, axioms::*},
    profiles::el_reasoner::{CompletionConfig, ELReasoner},
    reasoning::ReasoningService,
    server::{ServerManager, owllink::OWLlinkServer, rest::RestApiServer, sparql::SparqlServer},
};
...
async fn test_sparql_server_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sparql_server = SparqlServer::new(8001, "127.0.0.1".to_string(), reasoning_service);
...
async fn test_end_to_end_reasoning_with_explanations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Add individual
    let socrates = Individual::Named(crate::ontology::NamedIndividual::new(
...
async fn test_performance_benchmarks() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        el_duration < Duration::from_secs(5),
        "EL classification should complete within 5 seconds"
    );
    assert!(
        explanation_duration < Duration::from_secs(10),
        "Explanation generation should complete within 10 seconds"
    );
