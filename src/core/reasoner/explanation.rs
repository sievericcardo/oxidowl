...
impl ExplanationService {
    ...
    pub fn explain_unsatisfiability(&self, class: &ClassExpression, ontology: &Ontology) -> Result<Vec<Axiom>> {
        let mut explanation = Vec::new();
                ...
                Axiom::ClassAssertion(class_assertion) => {
                    if class_assertion.individual == *individual {
                        ...
                        if class_assertion.class == *class {
                            ...
                            let subsumption_explanation = self.explain_subsumption(
    ...
    fn format_axiom(&self, axiom: &Axiom) -> String {
        match axiom {
            Axiom::SubClassOf(subclass_axiom) => {
                format!("{:?} ⊑ {:?}", subclass_axiom.subclass, subclass_axiom.superclass)
            }
            Axiom::EquivalentClasses(equiv_axiom) => {
                let classes: Vec<String> = equiv_axiom.classes.iter()
                    .map(|c| format!("{:?}", c))
                    .collect();
                format!("EquivalentClasses({})", classes.join(", "))
            }
            Axiom::ClassAssertion(class_assertion) => {
                format!("{:?} ∈ {:?}", class_assertion.individual, class_assertion.class)
            }
            Axiom::ObjectPropertyAssertion(prop_assertion) => {
                format!("({:?}, {:?}) ∈ {:?}", 
            ...
            Axiom::DisjointClasses(disjoint_axiom) => {
                let classes: Vec<String> = disjoint_axiom.classes.iter()
                    .map(|c| format!("{:?}", c))
                    .collect();
