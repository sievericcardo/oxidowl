...
use super::ground_disjunction::GroundDisjunction;
...
impl ExtensionManager {
        ...
        match arity {
            ...
            _ => {
                self.extension_tables
                    .entry(arity)
                    .or_insert_with(|| ExtensionTable::new(arity));
    ...
    pub fn has_more_tuples(&self, retrieval_id: usize, arity: usize) -> Result<bool> {
        let table = self
            ...
            .get(&arity)
            .or(if arity == 2 {
                Some(&self.binary_extension_table)
            } else if arity == 3 {
                Some(&self.ternary_extension_table)
            } else {
                None
            })
    ...
    pub fn get_facts(&self, predicate: &str, view: &RetrievalView) -> Result<Vec<Vec<String>>> {
        ...
        results.extend(
            self.binary_extension_table
                .get_facts_for_predicate(predicate, view)?,
        );
    ...
    pub fn add_ground_disjunction(&mut self, disjunction: GroundDisjunction) -> Result<()> {
        ...
        self.add_fact(
            "GroundDisjunction".to_string(),
            vec![disjunction.to_string()],
        )?;
        Ok(())
    }
    ...
    #[must_use]
    pub fn contains_concept_assertion(
        &self,
        node_id: &str,
        concept: &crate::ontology::ClassExpression,
    ) -> bool {
        ...
        if let crate::ontology::ClassExpression::Class(class) = concept {
            ...
            if let Some(table) = self.get_table_for_arity(1) {
                return table
                    .has_fact(class_name, &[node_id.to_string()])
                    .unwrap_or(false);
            }
        }
    ...
    #[must_use]
    pub fn contains_role_assertion(
        &self,
        subj_id: &str,
        ...
        match property {
            crate::ontology::ObjectPropertyExpression::ObjectProperty(prop) => {
                ...
                if let Some(table) = self.get_table_for_arity(2) {
                    table
                        .has_fact(property_name, &[subj_id.to_string(), obj_id.to_string()])
                        .unwrap_or(false)
                } else {
                    false
                }
            ...
            crate::ontology::ObjectPropertyExpression::InverseObjectProperty(prop) => {
                ...
                if let Some(table) = self.get_table_for_arity(2) {
                    ...
                    table
                        .has_fact(property_name, &[obj_id.to_string(), subj_id.to_string()])
                        .unwrap_or(false)
                } else {
                    false
                }
    ...
    fn check_property_chain_simple(
        &self,
        subj_id: &str,
        obj_id: &str,
        ...
        if let Some(table) = self.get_table_for_arity(2) {
            ...
            if let Ok(first_facts) = table.get_facts_for_predicate(
                first_prop,
                &crate::core::hypertableau::extension_table::RetrievalView::Complete,
            ) {
                for fact in first_facts {
                    if fact.len() == 2 && fact[0] == subj_id {
                        ...
                        if table
                            .has_fact(second_prop, &[intermediate.clone(), obj_id.to_string()])
                            .unwrap_or(false)
                        {
                            return true;
                        }
    ...
    pub fn are_nodes_equal(&self, left_id: &str, right_id: &str) -> bool {
        ...
        if let Some(table) = self.get_table_for_arity(2) {
            return table
                .has_fact(equality_key, &[left_id.to_string(), right_id.to_string()])
                .unwrap_or(false)
                || table
                    .has_fact(equality_key, &[right_id.to_string(), left_id.to_string()])
                    .unwrap_or(false);
        }
                ...
                for fact in disjoint_table.get_all_facts() {
                    if fact.len() >= 3 && fact[0] == disjoint_key {
                        if let (Some(class1), Some(class2)) = (fact.get(1), fact.get(2)) {
                            ...
                            if let Some(table) = self.get_table_for_arity(arity) {
                                if class2 == predicate
                                    && table.has_fact(class1, args).unwrap_or(false)
                                {
                                    return true;
                                }
                                // If predicate is disjoint with class2, check if individual is in class2
                                if class1 == predicate
                                    && table.has_fact(class2, args).unwrap_or(false)
                                {
                                    return true;
                                }
                            }
                ...
                if fact.len() >= 3 && fact[0] == all_disjoint_key {
                    ...
                    if let Some(class_list_id) = fact.get(2) {
                        let class_list = self
                            ...
                            .parse_rdf_list_from_extension(class_list_id)?;
    ...
    fn is_concept_in_disjoint_list(&self, concept: &str, list_id: &str) -> bool {
        ...
        if let Ok(list_items) = self
            .binary_extension_table
            .parse_rdf_list_from_extension(list_id)
        {
    ...
    fn get_other_classes_in_disjoint_list(
        &self,
        exclude_concept: &str,
        list_id: &str,
    ) -> Vec<String> {
    ...
    pub fn are_nodes_unequal(&self, left_id: &str, right_id: &str) -> bool {
        ...
        if let Some(table) = self.get_table_for_arity(2) {
            if table
                .has_fact(inequality_key, &[left_id.to_string(), right_id.to_string()])
                .unwrap_or(false)
                || table
                    .has_fact(inequality_key, &[right_id.to_string(), left_id.to_string()])
                    .unwrap_or(false)
            {
                return true;
            }
        ...
        if let Some(binary_table) = self.get_table_for_arity(2) {
            for fact in binary_table.get_all_facts() {
                if fact.len() >= 3 {
                    ...
                    if !fact[1].starts_with("owl:")
                        && !fact[2].starts_with("owl:")
                        && !fact[1].starts_with("rdf:")
                        && !fact[2].starts_with("rdf:")
                    {
    ...
    pub fn add_concept_assertion_with_dependency(
        ...
        self.add_fact_with_dependencies(concept_name, vec![individual.to_string()], dependencies)
    }
...
impl ExtensionTable {
    ...
    pub fn open_retrieval(&mut self, retrieval_id: usize) -> Result<()> {
        ...
        if let Some(retrieval) = self.active_retrievals.get_mut(retrieval_id) {
            ...
            Err(Error::InvalidInput {
                message: "Invalid retrieval ID".to_string(),
            })
        }
    }
    ...
    pub fn get_facts_for_predicate(
        &self,
        predicate: &str,
        view: &RetrievalView,
    ...
    fn check_property_chain_recursive(
        &self,
        subj_id: &str,
        obj_id: &str,
        chain: &[crate::ontology::ObjectPropertyExpression],
        ...
        if let Some(table) = self.get_table_for_arity(2) {
            ...
            for fact in facts {
                if fact.len() >= 2 && fact[0] == subj_id {
                    ...
                    if self.check_property_chain_recursive(
                        intermediate_id,
                        obj_id,
                        chain,
                        position + 1,
        ...
        if let Some(table) = self.get_table_for_arity(2) {
            for fact in table.get_all_facts() {
                if fact.len() >= 3 && fact[0] == property_name {
                    if let (Ok(subj), Ok(obj)) =
                        (fact[1].parse::<usize>(), fact[2].parse::<usize>())
                    {
                        if subj == subj_id {
                            objects.push(obj);
                        }
    ...
    fn property_expression_to_string(
        &self,
        property: &crate::ontology::ObjectPropertyExpression,
    ) -> String {
        match property {
    ...
    fn has_object_property_fact(
        ...
        for tuple_entry in &self.tuples {
            ...
            {
                if let (Ok(subj), Ok(obj)) = (
                    tuple_entry.tuple[0].parse::<usize>(),
                    tuple_entry.tuple[1].parse::<usize>(),
                ) {
