...
use super::{
    dependency_tracking::{BranchingPointId, DependencyTracker},
    ground_disjunction::{DisjunctPredicate, GroundDisjunction},
};
...
impl BranchingManager {
    ...
    pub fn create_branching_point(
        ...
        if let Some(&parent_id) = self.current_path.last() {
            let parent_level = self.branching_points.get(&parent_id).map_or(0, |p| p.level);
        ...
        match self.strategy {
            ...
            BranchingStrategy::BestFirst => {
                ...
                let insert_pos = self
                    .choice_queue
                    .iter()
                    .position(|&other_id| self.branching_points[&other_id].priority < priority)
    ...
    pub fn get_next_branching_point(&mut self) -> Option<BranchingPointId> {
        match self.strategy {
            BranchingStrategy::DepthFirst => self.branching_stack.pop(),
            BranchingStrategy::BreadthFirst
            | BranchingStrategy::BestFirst
            | BranchingStrategy::Custom => self.choice_queue.pop_front(),
        }
    }
    ...
    pub fn make_choice(
        ...
        let branching_point = self.branching_points.get_mut(&branching_id).ok_or(
            Error::branching_point_not_found(format!("BranchingPoint({})", branching_id.0)),
        )?;
...
pub mod utils {
    use super::{
        BranchingChoice, ClassExpression, DisjunctPredicate, GroundDisjunction, Individual,
    };
    ...
    pub fn create_disjunction_choices(
        ...
        disjunction
            ...
            .filter_map(|(index, disjunct)| {
                ...
                match disjunct {
                    DisjunctPredicate::Concept { concept, .. } => Some(BranchingChoice::new(
                        index,
                    ...
                    DisjunctPredicate::Role {
                        ...
                        use crate::ontology::{Class, ClassExpression, IRI};
                    ...
                    DisjunctPredicate::Equality { left, right } => {
                        ...
                        let equality_concept =
                            ClassExpression::ObjectOneOf(vec![individual.clone()]);
                    ...
                    DisjunctPredicate::Inequality { left, right } => {
                        ...
                        let inequality_concept = ClassExpression::ObjectComplementOf(Box::new(
                            ClassExpression::ObjectOneOf(vec![individual.clone()]),
                        ));
    ...
    pub fn estimate_choice_cost(assertion: &ClassExpression) -> f64 {
        match assertion {
            ...
            ClassExpression::ObjectComplementOf(class) => estimate_choice_cost(class) + 1.5,
