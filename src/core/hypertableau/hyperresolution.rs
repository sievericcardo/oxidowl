...
impl HyperresolutionManager {
    ...
    fn add_compiled_clause_info(
        &mut self,
        predicate: String,
        mut info: CompiledDLClauseInfo,
    ) -> Result<()> {
        ...
        self.tuple_consumers_by_delta_predicate
            .insert(predicate, info);
        Ok(())
    }

    /// Add atomic role optimizations for guarded and unguarded cases
    fn add_atomic_role_optimizations(
        &mut self,
        predicate: String,
        info: CompiledDLClauseInfo,
    ) -> Result<()> {
        ...
        if guards.is_empty() {
            ...
            if let Some(existing) = self.atomic_role_consumers_unguarded.get(&predicate) {
                ...
                self.atomic_role_consumers_unguarded
                    .insert(predicate, new_info);
            } else {
            ...
            for guard_concept in guards {
                let guard_map = self
                    .atomic_role_consumers_by_guard_concept
    ...
    pub fn apply_dl_clauses(&mut self, extension_manager: &mut ExtensionManager) -> Result<()> {
        ...
        let predicates: Vec<(String, Vec<Vec<String>>)> = self
            .tuple_consumers_by_delta_predicate
            .keys()
            .map(|predicate| {
                let delta_tuples = extension_manager
                    .get_delta_old_tuples(predicate)
    ...
    fn should_use_optimization(&self, predicate: &str, tuple: &[String]) -> bool {
        ...
        self.atomic_role_consumers_by_guard_concept
    ...
    pub fn apply_rules(
        &mut self,
        extension_manager: &mut ExtensionManager,
        _branching_manager: &mut crate::core::hypertableau::branching::BranchingManager,
    ) -> crate::Result<bool> {
        ...
        match &self.operation {
            ...
            WorkerOperation::ApplyHead => {
                ...
                if self.arguments.len() >= 2 {
                    ...
                    let args: Vec<String> = self.arguments[1..]
                        .iter()
            ...
            WorkerOperation::Return => Ok(WorkerResult::Return),
        }
    }
}
