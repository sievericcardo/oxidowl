...
pub enum WorkerOperation {
    ...
    CheckAfterLast { fail_target: usize },

    /// Move to next tuple in retrieval
    NextTuple { loop_target: usize },
    ...
    CreateGroundDisjunction { atoms: Vec<String>, priority: i32 },
...
impl DLClauseEvaluator {
    ...
    fn compile_body_matching(&mut self) -> Result<()> {
        ...
        let atoms_data: Vec<(usize, Atom)> = self
            .body_clause
            .body
            .iter()
            .enumerate()
        ...
        for (atom_index, atom) in atoms_data {
            ...
            let binding_pattern: Vec<bool> = atom
                .arguments
                .iter()
            ...
            self.workers.push(Worker {
                operation: WorkerOperation::OpenRetrieval {
                    ...
                    view: if atom_index == 0 {
                        RetrievalView::DeltaOld
                    } else {
                        RetrievalView::Extension
                    },
                },
                next_pc: self.workers.len() + 1,
                jump_target: None,
                arguments: atom.arguments.clone(),
                variable_indices: atom
                    .arguments
                    .iter()
            ...
            self.workers.push(Worker {
                operation: WorkerOperation::CheckAfterLast { fail_target },
                next_pc: self.workers.len() + 1,
            ...
            self.workers.push(Worker {
                operation: WorkerOperation::NextTuple { loop_target },
                next_pc: self.workers.len() + 1,
    ...
    fn optimize_workers(&mut self) -> Result<()> {
        ...
        self.workers
            .retain(|worker| !matches!(worker.operation, WorkerOperation::Nop));
    ...
    fn execute_worker(
        ...
        match &worker.operation {
            ...
            WorkerOperation::NextTuple { loop_target } => self.next_tuple(*loop_target, context),
            ...
            WorkerOperation::AddDependency {
                source_indices,
                target_index,
            } => self.add_dependency(source_indices, *target_index, context),

            WorkerOperation::Jump { target } => Ok(ExecutionResult::Jump(*target)),

            WorkerOperation::Return => Ok(ExecutionResult::Return),

            WorkerOperation::Nop => Ok(ExecutionResult::Continue),
        }
    }
    ...
    fn apply_head(
        ...
        if let Some(head_clause) = self.head_clauses.get(clause_index) {
            if let Some(atom) = head_clause.head.get(atom_index) {
                ...
                context
                    .extension_manager
                    .add_fact(atom.predicate.clone(), args)?;
    ...
    fn create_ground_disjunction(
        ...
        use crate::core::hypertableau::ground_disjunction::{
            DisjunctPredicate, DisjunctionPriority, GroundDisjunctionHeader,
        };
        ...
        let predicates: Vec<DisjunctPredicate> = atoms
            .iter()
            .enumerate()
            .map(|(i, _atom)| {
                // Create a placeholder concept predicate for now
                DisjunctPredicate::Concept {
                    concept: crate::ontology::ClassExpression::Class(crate::ontology::Class {
                        iri: crate::ontology::IRI::new("http://example.org/placeholder")
                            .to_url()
                            .expect("Valid URL")
                            .into(),
                    }),
        ...
        let header =
            GroundDisjunctionHeader::new_with_predicates(predicates, DisjunctionPriority::Normal);
        ...
        context
            .extension_manager
            .add_ground_disjunction(disjunction)?;
        Ok(ExecutionResult::Application)
    }
    ...
    fn add_dependency(
        ...
        for &source_index in source_indices {
            if let Some(retrieval) = context.retrievals.get(source_index) {
                // Add dependency information
                dependencies =
                    dependencies.union(&DependencySet::singleton(source_index.to_string()));
            }
        }
        ...
        context
            .extension_manager
            .add_dependency(target_index.to_string(), dependencies)?;
        Ok(ExecutionResult::Continue)
    }
...
impl ValuesBufferManager {
    ...
    fn initialize(&mut self, variable_count: usize) {
        ...
        self.values_buffer.resize(variable_count, Some(crate::dl_clauses::Individual(0)));
        self.variable_mappings.clear();
    }
    
    /// Set value for a variable at specific index
    fn set_value(&mut self, variable_index: usize, value: crate::dl_clauses::Individual) -> std::result::Result<(), String> {
        if variable_index >= self.max_variables {
            return Err(format!("Variable index {} exceeds buffer size {}", variable_index, self.max_variables));
        }
    ...
    fn get_value(&self, variable_index: usize) -> std::result::Result<crate::dl_clauses::Individual, String> {
        if variable_index >= self.max_variables {
            return Err(format!("Variable index {} exceeds buffer size {}", variable_index, self.max_variables));
        }
        
        self.values_buffer[variable_index].ok_or_else(|| 
            format!("No value set for variable at index {}", variable_index)
        )
    }
    
    /// Create a mapping from variable to buffer index
    fn add_variable_mapping(&mut self, variable: crate::dl_clauses::Variable, index: usize) -> std::result::Result<(), String> {
        if index >= self.max_variables {
            return Err(format!("Buffer index {} exceeds maximum {}", index, self.max_variables));
        }
    ...
    fn set_variable_value(&mut self, variable: &crate::dl_clauses::Variable, value: crate::dl_clauses::Individual) -> std::result::Result<(), String> {
    ...
    fn get_variable_value(&self, variable: &crate::dl_clauses::Variable) -> std::result::Result<crate::dl_clauses::Individual, String> {
        if let Some(index) = self.get_variable_index(variable) {
