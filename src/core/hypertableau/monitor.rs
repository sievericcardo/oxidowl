...
use crate::{Error, ontology::Individual};

use super::{
    branching::BranchingStats,
    clause_evaluator::EvaluationStatistics,
    dependency_tracking::{BranchingPointId, DependencyStats},
...
impl EventListener for ConsoleEventListener {
    fn on_event(&self, event: &MonitoredEvent) {
        ...
        match self.level {
            MonitoringLevel::Basic => match event {
...
impl TableauMonitor {
    ...
    pub fn finish(&mut self) -> ReasoningStats {
        ...
        if let Some(reasoning_start) = self.reasoning_start_time {
            ...
            self.stats.cleanup_duration = self.stats.total_duration
                - self.stats.startup_duration
                - self.stats.reasoning_duration;
        }
    ...
    pub fn get_stats(&mut self) -> &ReasoningStats {
        ...
        let now = Instant::now();
        if now - self.performance_cache.last_calculation
            > self.performance_cache.calculation_interval
        {
            self.calculate_performance_metrics();
    ...
    fn get_event_type_name(&self, event: &MonitoredEvent) -> String {
        match event {
            ...
            MonitoredEvent::GroundDisjunctionProcessing { .. } => {
                "GroundDisjunctionProcessing".to_string()
            }
...
pub mod events {
    use super::{Duration, Individual, MonitoredEvent};
