...
use crate::{
    ...
    core::{
        ...
        tableau::{TableauEdge, TableauNode, TableauState},
    },
...
use ground_disjunction::{DisjunctPredicate, GroundDisjunction};
use hyperresolution::HyperresolutionManager;
use monitor::{MonitoringLevel, ReasoningStats, TableauMonitor};
...
impl HyperTableau {
    ...
    pub fn get_reasoning_stats(&mut self) -> ReasoningStats {
        ...
        self.monitor
            .update_dependency_stats(self.dependency_tracker.lock().unwrap().get_stats());
        self.monitor
            .update_branching_stats(self.branching_manager.get_stats());
        self.monitor
            .update_hyperresolution_stats(&self.hyperresolution_manager.get_statistics());
        self.monitor
            .update_extension_stats(self.extension_manager.get_statistics());
    ...
    fn apply_hyperresolution(&mut self) -> Result<bool> {
        ...
        let new_disjunctions = self
            .hyperresolution_manager
            .apply_rules(&mut self.extension_manager, &mut self.branching_manager)?;
    ...
    fn process_ground_disjunctions(&mut self) -> Result<bool> {
        ...
        while let Some(disjunction_id) = self.first_unprocessed_disjunction {
            ...
            let individual_iri =
                crate::ontology::IRI::new(&format!("http://example.org/{individual_str}"));
            let individual = Individual::named(individual_iri);

            // Log monitoring event
            self.monitor
                .log_event(monitor::events::ground_disjunction_processing(
                    format!("{disjunction:?}"),
                    individual,
    ...
    fn handle_ground_disjunction(&mut self, disjunction_id: usize) -> Result<bool> {
        ...
        if disjunction.disjuncts().len() == 1 {
            ...
            if let DisjunctPredicate::Concept { concept, .. } = &disjunction.disjuncts()[0] {
                self.extension_manager
                    .add_concept_assertion(&disjunction.individual(), concept)?;
        ...
        let choices = branching::utils::create_disjunction_choices(&disjunction, &individual);
    ...
    fn handle_clash(&mut self) -> Result<bool> {
        ...
        if let Some(branch_id) = self.branching_manager.backtrack()? {
                ...
                self.branching_manager.make_choice(branch_id, None)?
            {
                ...
                self.extension_manager
                    .add_concept_assertion(&individual.to_string(), &assertion)?;
                return Ok(true);
            }
        }
    ...
    fn apply_blocking(&mut self) -> Result<()> {
        ...
        let individuals: Vec<String> = self
            .extension_manager
            .get_all_individuals()
            .unwrap_or_default();
        ...
        for blocker in &individuals {
            for blocked in &individuals {
                if blocker != blocked && self.can_block(blocker, blocked)? {
                    self.extension_manager
                        .add_blocking(blocker.clone(), blocked.clone())?;
    ...
    fn compile_ontology_to_clauses(
        &self,
        ontology: &Ontology,
    ...
    fn compile_subclass_axiom(
    ...
    ) -> Result<hyperresolution::DLClause> {
        ...
        let superclass_atom =
            self.compile_class_expression_to_atom(&axiom.superclass, &var_x, false)?; // positive
    ...
    fn compile_object_property_assertion(
        &self,
    ...
    fn compile_class_expression_to_atom(
        ...
        variable: &str,
        negated: bool,
    ) -> Result<hyperresolution::Atom> {
        match expr {
            ClassExpression::Class(class) => Ok(hyperresolution::Atom {
        ...
        for axiom in ontology.axioms() {
            match axiom {
                Axiom::ClassAssertion(class_assertion) => {
                    ...
                    self.extension_manager
                        .ensure_individual_exists(&individual_name)?;
                }
                Axiom::ObjectPropertyAssertion(prop_assertion) => {
                    ...
                    self.extension_manager
                        .ensure_individual_exists(&subject_name)?;
                    self.extension_manager
                        .ensure_individual_exists(&object_name)?;
                }
                _ => {
        ...
        for axiom in ontology.axioms() {
            match axiom {
                Axiom::ClassAssertion(class_assertion) => {
                    ...
                    self.extension_manager
                        .add_concept_assertion(&individual_name, &class_assertion.class)?;
                ...
                Axiom::ObjectPropertyAssertion(prop_assertion) => {
                    ...
                    self.extension_manager.add_role_assertion(
                        &subject_name,
...
mod tests {
    ...
    fn test_monitoring_level_setting() {
        ...
        assert_eq!(
            tableau.monitor.get_monitoring_level(),
