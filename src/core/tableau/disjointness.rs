...
use crate::ontology::{Ontology, Axiom, ClassExpression};
...
impl DisjointnessMap {
    ...
    pub fn from_ontology(ontology: &Ontology, _eq_closure: &EquivalenceClosure) -> Result<Self> {
        ...
        for axiom in ontology.axioms() {
            match axiom {
                ...
                Axiom::DisjointClasses(disj) => {
                    log::trace!("Processing DisjointClasses with {} classes", disj.classes.len());
                ...
                Axiom::DisjointUnion(du) => {
                    ...
                    let children: Vec<ConceptId> = du.disjoint_classes.iter()
                    ...
                    for i in 0..children.len() {
                        for j in (i + 1)..children.len() {
                            log::trace!("Adding disjoint pair from union: {:?} ⊥ {:?}", 
        ...
        log::info!("Built disjointness map with {} disjoint pairs and {} disjoint unions",
                   map.disjoint_pairs.len(), map.disjoint_unions.len());
                ...
                for j in (i + 1)..members_vec.len() {
                    if self.are_disjoint(members_vec[i], members_vec[j]) {
                        ...
                        log::warn!(
                            ...
                            members_vec[i], members_vec[j], members_vec[i], members_vec[j]
                        );
