...
impl IncrementalClassifier {
    ...
    pub fn initialize(&self, ontology: &Ontology) -> Result<()> {
        ...
        let index = self.concept_index.write().map_err(|e| Error::Internal {
        ...
        // Update stats
        let mut stats = self.stats.write().map_err(|e| Error::Internal {
    ...
    pub fn add_dependency(
        ...
        let mut deps = self.dependencies.write().map_err(|e| Error::Internal {
    ...
    pub fn get_affected_concepts(&self, changed_iris: &[String]) -> Result<HashSet<u64>> {
        let index = self.concept_index.read().map_err(|e| Error::Internal {
            message: format!("IncrementalClassifier index lock poisoned: {e}"),
        })?;
        ...
        let deps = self.dependencies.read().map_err(|e| Error::Internal {
            message: format!("IncrementalClassifier dependencies lock poisoned: {e}"),
        })?;
    ...
    pub fn update_classification(
        ...
        let mut classification = self.classification.write().map_err(|e| Error::Internal {
    ...
    pub fn get_classification(&self, concept_hash: u64) -> Result<Option<HashSet<u64>>> {
        let classification = self.classification.read().map_err(|e| Error::Internal {
            message: format!("IncrementalClassifier classification lock poisoned: {e}"),
        })?;
    ...
    pub fn statistics(&self) -> Result<IncrementalStatistics> {
        let stats = self.stats.read().map_err(|e| Error::Internal {
            message: format!("IncrementalClassifier stats lock poisoned: {e}"),
        })?;
        Ok(stats.clone())
    }
...
mod tests {
    ...
    fn test_classification_storage() {
        ...
        classifier
            .update_classification(person_hash, super_classes.clone())
            .unwrap();
    ...
    fn test_incremental_statistics() {
        ...
        classifier
            .update_classification(1, HashSet::from([2]))
            .unwrap();
