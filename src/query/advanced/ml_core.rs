...
impl MLHeuristicsEngine {
    ...
    pub fn predict_cost(&self, features: &QueryFeatures) -> Result<CostPrediction, Error> {
        let predictor = self.cost_predictor.read()
            .map_err(|e| Error::Internal { message: format!("Failed to acquire predictor lock: {}", e) })?;
    ...
    pub fn select_strategy(&self, features: &QueryFeatures) -> Result<StrategyRecommendation, Error> {
        let selector = self.strategy_selector.read()
            .map_err(|e| Error::Internal { message: format!("Failed to acquire selector lock: {}", e) })?;
    ...
    pub fn add_training_data(&self, execution: QueryExecution) -> Result<(), Error> {
        let mut data = self.training_data.write()
            .map_err(|e| Error::Internal { message: format!("Failed to acquire training data lock: {}", e) })?;
        
        data.add(execution);
        
        // Check if we should trigger model update
        if data.size() >= self.config.min_training_samples && data.size() % self.config.training_batch_size == 0 {
    ...
    pub fn train_model(&self) -> Result<TrainingMetrics, Error> {
        let data = self.training_data.read()
            .map_err(|e| Error::Internal { message: format!("Failed to acquire training data lock: {}", e) })?;
        ...
        if training_samples.is_empty() {
            return Err(Error::Internal { message: "No training samples available".to_string() });
        }
        
        // Train cost prediction model
        let metrics = {
            let mut predictor = self.cost_predictor.write()
                .map_err(|e| Error::Internal { message: format!("Failed to acquire predictor lock: {}", e) })?;
    ...
    #[cfg(not(feature = "ml"))]
    pub fn train_model(&self) -> Result<TrainingMetrics, Error> {
        Err(Error::Internal { message: "ML feature not enabled".to_string() })
    }
    ...
    pub fn load_models(&self) -> Result<(), Error> {
        ...
        if let Ok(model) = self.model_storage.load_strategy_selector() {
            let mut selector = self.strategy_selector.write()
                .map_err(|e| Error::Internal { message: format!("Failed to acquire selector lock: {}", e) })?;
            *selector = Some(model);
        }
...
impl QueryFeatureExtractor {
    ...
    pub fn extract(
        ...
        let signature = ontology.signature().unwrap_or_else(|_| crate::ontology::Signature::new());
        let property_count = (ontology.object_properties().len() + signature.data_properties.len() + 1) as f32;
    ...
    fn count_atom_types(&self, query: &ConjunctiveQuery) -> (f32, f32) {
        ...
        for atom in &query.body_atoms {
            match atom {
                ...
                QueryAtom::ObjectPropertyAtom { .. } | QueryAtom::DataPropertyAtom { .. } => property_atoms += 1,
                _ => {}
            }
        }
    ...
    fn get_similar_query_time(&self, query_hash: u64) -> Result<f32, Error> {
        let history = self.query_history.read()
            .map_err(|e| Error::Internal { message: format!("Failed to read query history: {}", e) })?;
        
        // Find similar queries (simplified - just look for exact match)
        let similar_times: Vec<f32> = history.iter()
    ...
    pub fn record_execution(&self, query_hash: u64, execution_time: f32) -> Result<(), Error> {
        let mut history = self.query_history.write()
            .map_err(|e| Error::Internal { message: format!("Failed to write query history: {}", e) })?;
...
impl CostPrediction {
    ...
    pub fn baseline(features: &QueryFeatures) -> Self {
        ...
        let execution_time = (features.atom_count as f64 
            * features.ontology_size as f64).ln().max(0.1);
...
impl CostPredictionModel {
    ...
    pub fn predict(&self, features: &QueryFeatures) -> Result<CostPrediction, Error> {
        ...
        let input = Tensor::from_vec(feature_vec, &[1, 18], &self.device)
            .map_err(|e| Error::Internal { message: format!("Failed to create input tensor: {}", e) })?;
        
        // Forward pass with ReLU activations
        let x = self.layer1.forward(&input)
            .map_err(|e| Error::Internal { message: format!("Layer1 forward failed: {}", e) })?
            .relu()
            .map_err(|e| Error::Internal { message: format!("ReLU failed: {}", e) })?;
        
        let x = self.layer2.forward(&x)
            .map_err(|e| Error::Internal { message: format!("Layer2 forward failed: {}", e) })?
            .relu()
            .map_err(|e| Error::Internal { message: format!("ReLU failed: {}", e) })?;
        
        let x = self.layer3.forward(&x)
            .map_err(|e| Error::Internal { message: format!("Layer3 forward failed: {}", e) })?
            .relu()
            .map_err(|e| Error::Internal { message: format!("ReLU failed: {}", e) })?;
        
        let output = self.output.forward(&x)
            .map_err(|e| Error::Internal { message: format!("Output forward failed: {}", e) })?;
        
        let prediction = output.to_vec1::<f32>()
            .map_err(|e| Error::Internal { message: format!("Failed to extract prediction: {}", e) })?[0];
    ...
    pub fn train(&mut self, samples: &[TrainingSample], learning_rate: f64) -> Result<TrainingMetrics, Error> {
...
impl StrategySelectionModel {
    ...
    pub fn select(&self, features: &QueryFeatures) -> Result<StrategyRecommendation, Error> {
        ...
        let (strategy, confidence, reasoning) = match pattern {
            ...
            QueryPattern::Complex => {
                ...
                if result_estimate <= 100.0 {
                    (ExecutionStrategy::Adaptive, 0.70,
                     "Complex query with small result - adaptive strategy")
                } else {
                    (ExecutionStrategy::Hybrid, 0.72,
        ...
        let expected_performance = self.strategy_registry
            .get(&strategy)
    ...
    fn detect_query_pattern(&self, features: &QueryFeatures) -> QueryPattern {
        ...
        if features.join_count >= 2.0 && features.join_count <= 5.0 
...
impl ModelStorage {
    pub fn new(storage_dir: PathBuf) -> Result<Self, Error> {
        std::fs::create_dir_all(&storage_dir)
            .map_err(|e| Error::Internal { message: format!("Failed to create storage directory: {}", e) })?;
    ...
    pub fn load_cost_predictor(&self) -> Result<CostPredictionModel, Error> {
        ...
        Err(Error::Internal { message: "No saved model found".to_string() })
    }
    ...
    pub fn save_strategy_selector(&self, model: &StrategySelectionModel) -> Result<(), Error> {
        ...
        let data = bincode::serialize(model)
            .map_err(|e| Error::Internal { message: format!("Failed to serialize model: {}", e) })?;
        
        std::fs::write(path, data)
            .map_err(|e| Error::Internal { message: format!("Failed to write model: {}", e) })?;
    ...
    pub fn load_strategy_selector(&self) -> Result<StrategySelectionModel, Error> {
        ...
        let data = std::fs::read(path)
            .map_err(|e| Error::Internal { message: format!("Failed to read model: {}", e) })?;
        
        bincode::deserialize(&data)
            .map_err(|e| Error::Internal { message: format!("Failed to deserialize model: {}", e) })
    }
}
...
mod tests {
    ...
    fn test_strategy_selection_model_new() {
        ...
        assert!(model.strategy_registry.contains_key(&ExecutionStrategy::IndexedLookup));
        assert!(model.strategy_registry.contains_key(&ExecutionStrategy::JoinOrder));
        assert!(model.strategy_registry.contains_key(&ExecutionStrategy::Materialization));
        assert!(model.strategy_registry.contains_key(&ExecutionStrategy::Hybrid));
        assert!(model.strategy_registry.contains_key(&ExecutionStrategy::Default));
    }
    ...
    fn test_strategy_metadata_registry() {
        ...
        let indexed = model.strategy_registry.get(&ExecutionStrategy::IndexedLookup).unwrap();
        assert_eq!(indexed.strategy.as_str(), "indexed_lookup");
        assert!(indexed.resources.requires_index);
        assert_eq!(indexed.performance.scalability, ScalabilityClass::Logarithmic);
        assert!(indexed.success_rate > 0.9);
        
        // Verify JoinOrder metadata
        let join_order = model.strategy_registry.get(&ExecutionStrategy::JoinOrder).unwrap();
        assert_eq!(join_order.strategy.as_str(), "join_order");
        assert!(join_order.resources.supports_parallel);
        assert_eq!(join_order.performance.scalability, ScalabilityClass::Linearithmic);
        
        // Verify Materialization metadata
        let materialization = model.strategy_registry.get(&ExecutionStrategy::Materialization).unwrap();
        assert_eq!(materialization.strategy.as_str(), "materialization");
        assert_eq!(materialization.performance.scalability, ScalabilityClass::Linear);
        
        // Verify Hybrid metadata
        let hybrid = model.strategy_registry.get(&ExecutionStrategy::Hybrid).unwrap();
    ...
    fn test_execution_strategy_as_str() {
        ...
        assert_eq!(ExecutionStrategy::Materialization.as_str(), "materialization");
        assert_eq!(ExecutionStrategy::Hybrid.as_str(), "hybrid");
        assert_eq!(ExecutionStrategy::BackwardChaining.as_str(), "backward_chaining");
        assert_eq!(ExecutionStrategy::ForwardChaining.as_str(), "forward_chaining");
