...
impl DLQueryFeatureExtractor {
    ...
    fn count_restrictions(&self, query: &ConjunctiveQuery) -> (usize, usize) {
        ...
        for atom in &query.body_atoms {
            ...
            {
                self.count_restrictions_recursive(
                    class_expression,
                    &mut existential,
                    &mut universal,
                );
    ...
    fn count_restrictions_recursive(
        &self,
        expr: &ClassExpression,
        existential: &mut usize,
        universal: &mut usize,
    ) {
        match expr {
            ClassExpression::ObjectSomeValuesFrom { filler, .. } => {
    ...
    fn count_boolean_operators(&self, query: &ConjunctiveQuery) -> (usize, usize, usize) {
        ...
        for atom in &query.body_atoms {
            ...
            {
                self.count_boolean_operators_recursive(
                    class_expression,
                    &mut intersections,
                    &mut unions,
                    &mut complements,
    ...
    fn test_query_hash() {
        ...
        let query = ConjunctiveQuery {
            ...
            body_atoms: vec![QueryAtom::ClassAtom {
                class_expression: ClassExpression::Class(Class::new(IRI::new(
