...
use crate::{
    ...
    core::{
        ...
        tableau::{Tableau, TableauBuilder, TableauState, TableauNode},
    },
    Error, Result,
    ontology::{
        Axiom, ClassExpression, Individual, Ontology, OntologyRef,
        ObjectPropertyExpression, DataPropertyExpression, DataRange,
        IRI, Literal
    },
};
...
impl Owl2Interpretation {
    ...
    fn initialize_owl_vocabulary(&mut self) {
        ...
        self.class_interpretation.insert(OWL_NOTHING.to_string(), HashSet::new());
    ...
    pub fn set_domain(&mut self, domain: HashSet<String>) {
        ...
        self.class_interpretation.insert(OWL_THING.to_string(), domain);
    }
    ...
    pub fn set_object_property_interpretation(&mut self, property: String, pairs: HashSet<(String, String)>) {
        self.object_property_interpretation.insert(property, pairs);
    }

    /// Set data property interpretation
    pub fn set_data_property_interpretation(&mut self, property: String, pairs: HashSet<(String, String)>) {
    ...
    pub fn set_individual_interpretation(&mut self, individual: String, domain_element: String) {
        self.individual_interpretation.insert(individual, domain_element);
    }
    ...
    pub fn interpret_class_expression(&self, expr: &ClassExpression) -> Result<HashSet<String>> {
        match expr {
            ...
            ClassExpression::ObjectOneOf(individuals) => {
                ...
                for individual in individuals {
                    if let Individual::Named(named) = individual {
                        if let Some(domain_element) = self.individual_interpretation.get(&named.iri.to_string()) {
            ...
            ClassExpression::ObjectMinCardinality { cardinality, property, filler } => {
                self.interpret_cardinality_restriction(*cardinality, property, Some(filler.as_ref()), CardinalityType::Min)
            }
            
            ClassExpression::ObjectMaxCardinality { cardinality, property, filler } => {
                self.interpret_cardinality_restriction(*cardinality, property, Some(filler.as_ref()), CardinalityType::Max)
            }
            
            ClassExpression::ObjectExactCardinality { cardinality, property, filler } => {
                self.interpret_cardinality_restriction(*cardinality, property, Some(filler.as_ref()), CardinalityType::Exact)
            }
            ...
            ClassExpression::DataMinCardinality { cardinality, property, filler } => {
                self.interpret_data_cardinality_restriction(*cardinality, property, Some(filler), CardinalityType::Min)
            }
            
            ClassExpression::DataMaxCardinality { cardinality, property, filler } => {
                self.interpret_data_cardinality_restriction(*cardinality, property, Some(filler), CardinalityType::Max)
            }
            
            ClassExpression::DataExactCardinality { cardinality, property, filler } => {
                self.interpret_data_cardinality_restriction(*cardinality, property, Some(filler), CardinalityType::Exact)
            }
        }
    }

    /// Interpret an object property expression
    pub fn interpret_object_property_expression(&self, expr: &ObjectPropertyExpression) -> Result<HashSet<(String, String)>> {
        match expr {
            ObjectPropertyExpression::ObjectProperty(prop) => {
                Ok(self.object_property_interpretation
            ...
            ObjectPropertyExpression::InverseObjectProperty(prop) => {
                let forward_interp = self.object_property_interpretation
                    .get(&prop.iri.to_string())
                    .cloned()
                    .unwrap_or_default();
            ...
            ObjectPropertyExpression::PropertyChain(chain) => {
                self.interpret_property_chain(chain)
            }
        }
    }

    /// Interpret a property chain
    fn interpret_property_chain(&self, chain: &[ObjectPropertyExpression]) -> Result<HashSet<(String, String)>> {
        if chain.is_empty() {
        ...
        match axiom {
            ...
            Axiom::ObjectPropertyAssertion(axiom) => {
                ...
                let source_interp = match &axiom.source {
                    Individual::Named(named) => self.individual_interpretation.get(&named.iri.to_string()).cloned(),
    ...
    fn interpret_data_range(&self, range: &DataRange) -> Result<HashSet<String>> {
        match range {
            ...
            DataRange::DataOneOf(datatype) => {
                // Return all values of the given datatype in our interpretation
                Ok(self.get_datatype_values(&datatype.first().map(|l| l.value.clone()).unwrap_or_default()))
            },
            DataRange::DataIntersectionOf(ranges) => {
                ...
                for r in ranges {
                    ...
                    result = match result {
                        ...
                        Some(acc) => Some(acc.intersection(&range_interpretation).cloned().collect()),
                    };
                }
            ...
            DataRange::DataComplementOf(range) => {
                ...
                Ok(all_values.difference(&range_interpretation).cloned().collect())
            },
    ...
    fn value_matches_datatype(&self, value: &str, datatype_iri: &str) -> bool {
        match datatype_iri {
            ...
            "http://www.w3.org/2001/XMLSchema#dateTime" => {
                ...
                value.contains('T') && (value.contains('+') || value.contains('-') || value.ends_with('Z'))
            },
    ...
    fn get_string_values(&self) -> HashSet<String> {
        self.get_all_data_values()
            .into_iter()
            .filter(|v| !v.parse::<i64>().is_ok() && !v.parse::<f64>().is_ok() && v != "true" && v != "false")
            .collect()
    }
        ...
        if let Some(predicate_str) = triple.predicate.as_str() {
            match predicate_str {
                "http://www.w3.org/1999/02/22-rdf-syntax-ns#type" => {
                    ...
                    if let (RdfTerm::Iri(individual), RdfTerm::Iri(class)) = (&triple.subject, &triple.object) {
                ...
                predicate_iri => {
                    ...
                    if let Some(prop_ext) = self.object_property_interpretation.get(predicate_iri) {
                        if let (RdfTerm::Iri(subj), RdfTerm::Iri(obj)) = (&triple.subject, &triple.object) {
                    ...
                    if let Some(prop_ext) = self.data_property_interpretation.get(predicate_iri) {
                        if let (RdfTerm::Iri(subj), RdfTerm::Literal { value, .. }) = (&triple.subject, &triple.object) {
...
impl SemanticInterpretation for Owl2Interpretation {
    ...
    fn interpret_term(&self, term: &RdfTerm) -> Option<String> {
        match term {
            RdfTerm::Iri(iri) => self.individual_interpretation.get(&iri.to_string()).cloned(),
...
impl Owl2ReasoningEngine {
    ...
    pub fn is_subsumed_by(&mut self, subclass: &ClassExpression, superclass: &ClassExpression) -> Result<bool> {
    ...
    pub fn are_equivalent(&mut self, class1: &ClassExpression, class2: &ClassExpression) -> Result<bool> {
    ...
    fn tableau_satisfiability_check(&self, class_expr: &ClassExpression) -> Result<bool> {
        ...
        while let Some(current_individual) = queue.pop_front() {
            ...
            // Apply tableau rules
            if self.apply_tableau_rules(&current_individual, &mut nodes, &mut queue, &mut next_individual_id)? {
        ...
        for concept in current_node.concepts.iter() {
            match concept {
                ...
                ClassExpression::ObjectSomeValuesFrom { property, filler } => {
                    ...
                    let has_successor = if let Some(successors) = current_node.role_successors.get(&property_name) {
    ...
    fn get_object_property_name(&self, property: &ObjectPropertyExpression) -> String {
        match property {
            ...
            ObjectPropertyExpression::PropertyChain(chain) => {
                let chain_names: Vec<String> = chain.iter()
...
mod tests {
    ...
    fn test_owl2_interpretation() {
        ...
        let owl_thing_interp = interp.class_interpretation.get(&OWL_THING.to_string()).unwrap();
        ...
        let owl_nothing_interp = interp.class_interpretation.get(&OWL_NOTHING.to_string()).unwrap();
