...
use crate::ontology::{OntologyRef, IRI};
...
impl OntologySegmenter {
    ...
    pub fn new(manager: Arc<RwLock<OntologyManager>>, module_type: ModuleType) -> Self {
        let config = ModuleExtractorConfig { module_type, max_iterations: 1000 };
        Self {
    ...
    pub fn segment(
        ...
        let iri = guard.get_iri().cloned().unwrap_or_else(|| IRI::new("urn:module"));
        drop(guard);

        let mut manager = self.manager.write().map_err(|e| crate::Error::Internal { message: format!("{e}") })?;
    ...
    pub fn segment_by_entities(
        ...
        let signature: HashSet<IRI> = entities.iter()
            .map(|e| e.iri().clone())
            .collect();
