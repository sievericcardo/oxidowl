...
impl AxiomCompiler for super::generator::DLClauseGenerator {
    ...
    fn compile_disjoint_union_axiom(&mut self, axiom: &crate::ontology::DisjointUnionAxiom) -> Result<Vec<DLClause>> {
        ...
        for union_class in &axiom.disjoint_classes {
            ...
            let class_atom_back = self.compile_class_expression_to_atom(&axiom.class, &var_x, false)?;
        ...
        for i in 0..axiom.disjoint_classes.len() {
            for j in (i + 1)..axiom.disjoint_classes.len() {
                ...
                let ci_atom = self.compile_class_expression_to_atom(&axiom.disjoint_classes[i], &var_x_disj, true)?;
                let cj_atom = self.compile_class_expression_to_atom(&axiom.disjoint_classes[j], &var_x_disj, true)?;
    ...
    fn compile_data_property_assertion_axiom(&mut self, axiom: &crate::ontology::DataPropertyAssertionAxiom) -> Result<Vec<DLClause>> {
        ...
        let property_atom = DLAtom::datatype_assertion(&property_name, &subject_name, &value_string);
    ...
    fn compile_sub_object_property_axiom(&mut self, axiom: &crate::ontology::SubObjectPropertyOfAxiom) -> Result<Vec<DLClause>> {
        ...
        match &axiom.sub_property {
            ...
            crate::ontology::ObjectPropertyExpression::InverseObjectProperty(inv_prop) => {
                ...
                let sub_prop_name = self.object_property_expression_to_string(&crate::ontology::ObjectPropertyExpression::ObjectProperty(inv_prop.clone()));
                let super_prop_name = self.object_property_expression_to_string(&axiom.super_property);
    ...
    fn compile_swrl_rule_axiom(&mut self, axiom: &crate::ontology::SWRLRuleAxiom) -> Result<Vec<DLClause>> {
        ...
        for body_atom in &axiom.rule.body {
            match body_atom {
                crate::ontology::SWRLAtom::ClassAtom { predicate, argument } => {
                    let arg_string = self.swrl_argument_to_string(argument);
                    let class_atom_dl = self.compile_class_expression_to_atom(predicate, &arg_string, true)?;
                    body_atoms.push(class_atom_dl);
                }
                crate::ontology::SWRLAtom::ObjectPropertyAtom { predicate, first_argument, second_argument } => {
                    ...
                    body_atoms.push(DLAtom::role_assertion(&prop_name, &arg1_string, &arg2_string));
                }
                crate::ontology::SWRLAtom::DataPropertyAtom { predicate, first_argument, second_argument } => {
                    let arg1_string = self.swrl_argument_to_string(first_argument);
                    ...
                    body_atoms.push(DLAtom::datatype_assertion(&prop_name, &arg1_string, &arg2_string));
                }
                crate::ontology::SWRLAtom::SameIndividualAtom { first_argument, second_argument } => {
                    let arg1_string = self.swrl_argument_to_string(first_argument);
                    let arg2_string = self.swrl_argument_to_string(second_argument);
                    body_atoms.push(DLAtom::new(format!("[{} == {}]", arg1_string, arg2_string), vec![]));
                }
                crate::ontology::SWRLAtom::DifferentIndividualsAtom { first_argument, second_argument } => {
                    let arg1_string = self.swrl_argument_to_string(first_argument);
                    let arg2_string = self.swrl_argument_to_string(second_argument);
                    // Different individuals as negated equality in body means this is a constraint
                    body_atoms.push(DLAtom::new(format!("[{} == {}]", arg1_string, arg2_string), vec![]));
                }
                crate::ontology::SWRLAtom::BuiltInAtom { predicate, arguments } => {
                    ...
                    body_atoms.push(DLAtom::new(format!("{}({})", builtin_name, args.join(", ")), args));
                }
                crate::ontology::SWRLAtom::DataRangeAtom { predicate, argument } => {
                    // Data range atom: D(z) - check if a data value belongs to a data range
                    let arg_string = self.swrl_dargument_to_string(argument);
                    let range_constraint = self.compile_data_range_to_constraint(predicate, &arg_string)?;
        ...
        for head_atom in &axiom.rule.head {
            match head_atom {
                crate::ontology::SWRLAtom::ClassAtom { predicate, argument } => {
                    let arg_string = self.swrl_argument_to_string(argument);
                    let class_atom_dl = self.compile_class_expression_to_atom(predicate, &arg_string, false)?;
                ...
                crate::ontology::SWRLAtom::ObjectPropertyAtom { predicate, first_argument, second_argument } => {
                    ...
                    let arg2_string = self.swrl_argument_to_string(second_argument);
                    let prop_name = self.object_property_expression_to_string(predicate);
                    head_atoms.push(DLAtom::role_assertion(&prop_name, &arg1_string, &arg2_string));
                }
                crate::ontology::SWRLAtom::DataPropertyAtom { predicate, first_argument, second_argument } => {
                    let arg1_string = self.swrl_argument_to_string(first_argument);
                    let arg2_string = self.swrl_dargument_to_string(second_argument);
                    let prop_name = self.data_property_expression_to_string(predicate);
                    head_atoms.push(DLAtom::datatype_assertion(&prop_name, &arg1_string, &arg2_string));
                }
                crate::ontology::SWRLAtom::SameIndividualAtom { first_argument, second_argument } => {
                    let arg1_string = self.swrl_argument_to_string(first_argument);
                    let arg2_string = self.swrl_argument_to_string(second_argument);
                    head_atoms.push(DLAtom::new(format!("[{} == {}]", arg1_string, arg2_string), vec![]));
                }
                crate::ontology::SWRLAtom::DifferentIndividualsAtom { first_argument, second_argument } => {
                ...
                crate::ontology::SWRLAtom::BuiltInAtom { predicate, arguments } => {
                    ...
                    head_atoms.push(DLAtom::new(format!("{}({})", builtin_name, args.join(", ")), args));
                }
                crate::ontology::SWRLAtom::DataRangeAtom { predicate, argument } => {
