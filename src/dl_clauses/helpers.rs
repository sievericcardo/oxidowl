...
pub trait HelperMethods {
    ...
    fn compile_class_expression_to_atom(&mut self, expr: &ClassExpression, variable: &str, is_body: bool) -> Result<DLAtom>;
    ...
    fn create_at_least_atom(&mut self, cardinality: u32, property: &str, range: &str, variable: &str, is_negative: bool) -> Result<DLAtom>;

    /// Create nominal atom for hasValue expressions
    fn create_nominal_atom(&mut self, value: &str, property: &str, variable: &str, is_negative: bool) -> Result<DLAtom>;
...
impl HelperMethods for super::generator::DLClauseGenerator {
    ...
    fn compile_class_expression_to_atom(&mut self, expr: &ClassExpression, variable: &str, is_body: bool) -> Result<DLAtom> {
        match expr {
            ...
            ClassExpression::ObjectIntersectionOf(operands) => {
                let intersection_parts: Vec<String> = operands.iter()
            ...
            ClassExpression::ObjectUnionOf(operands) => {
                let union_parts: Vec<String> = operands.iter()
                    .map(|op| self.class_expression_to_simple_string(op))
                    .collect();
    ...
    fn data_range_to_string(&self, range: &DataRange) -> String {
        match range {
            ...
            DataRange::DataOneOf(literals) => {
                let literal_values: Vec<String> = literals.iter()
    ...
    fn create_nominal_atom(&mut self, value: &str, _property: &str, variable: &str, is_negative: bool) -> Result<DLAtom> {
        ...
        match expr {
            ClassExpression::ObjectIntersectionOf(conjuncts) => {
                ...
                for conjunct in conjuncts {
                    let conjunct_atom = self.compile_class_expression_to_atom(conjunct, variable, false)?;
                ...
                for conjunct in conjuncts {
                    let conjunct_atom = self.compile_class_expression_to_atom(conjunct, variable, true)?;
...
impl super::generator::DLClauseGenerator {
    ...
    pub fn class_expression_to_simple_string(&self, expr: &ClassExpression) -> String {
        match expr {
            ...
            ClassExpression::ObjectIntersectionOf(operands) => {
                let parts: Vec<String> = operands.iter()
                    .map(|op| self.class_expression_to_simple_string(op))
                    .collect();
                format!("({})", parts.join(" ⊓ "))
            }
