...
pub trait UnionDisjunctiveCompiler: HelperMethods {
    ...
    fn compile_union_disjunctive_clauses(&mut self, union_classes: &[ClassExpression], var_x: &str) -> Result<Vec<DLClause>> {
    ...
    fn compile_union_member_expansions(&mut self, class_expr: &ClassExpression, var_x: &str) -> Result<Vec<DLClause>> {
        let mut clauses = Vec::new();
        ...
        match class_expr {
            ClassExpression::ObjectSomeValuesFrom { property, filler } => {
                ...
                let exists_atom_positive = self.compile_class_expression_to_atom(class_expr, var_x, false)?;
                let mut property_atom_negative = DLAtom::role_assertion(&property_name, var_x, &var_y);
                property_atom_negative.is_positive = false;
                let filler_atom_negative = self.compile_class_expression_to_atom(filler, &var_y, true)?;
            ...
            ClassExpression::ObjectAllValuesFrom { property, filler } => {
                ...
                let var_y = self.fresh_variable();
                let property_name = self.object_property_expression_to_string(property);
                let forall_atom = self.compile_class_expression_to_atom(class_expr, var_x, false)?;
            ...
            ClassExpression::ObjectMinCardinality { cardinality, property, filler } => {
                ...
                if *cardinality > 1 {
                    clauses.extend(self.compile_cardinality_witness_clauses(*cardinality, &property_name, Some(filler), var_x)?);
                }
            }
            ...
            ClassExpression::DataHasValue { property, value } => {
                ...
                if literal_value.contains("@") || literal_value.contains("WPS") || literal_value.contains("R365") || literal_value.contains("R385") {
                    let class_atom = self.compile_class_expression_to_atom(class_expr, var_x, true)?;
                    let var_y = self.fresh_variable();
                    let property_atom = DLAtom::role_assertion(&property_name, var_x, &var_y);
                    let nominal_atom = self.create_nominal_atom(literal_value, &property_name, &var_y, false)?;
            ...
            ClassExpression::ObjectComplementOf(complement) => {
                ...
                let class_atom = self.compile_class_expression_to_atom(class_expr, var_x, true)?;
                let complement_atom = self.compile_class_expression_to_atom(complement, var_x, true)?;
    ...
    fn compile_cardinality_witness_clauses(&mut self, cardinality: u32, property: &str, filler: Option<&ClassExpression>, var_x: &str) -> Result<Vec<DLClause>> {
        let mut clauses = Vec::new();
                ...
                if let Some(filler_expr) = filler {
                    ...
                    if i > 0 {
                        for j in 0..i {
                            ...
                            let inequality_atom = DLAtom::new(format!("[{} != {}]", var_y, var_z), vec![]);
