...
impl ManchesterParser {
    ...
    fn parse_prefix_declaration(&mut self, line: &str) -> Result<(), OxidowlError> {
        ...
        if parts.len() >= 3 {
            ...
            self.prefixes.insert(prefix_name.to_string(), iri.to_string());
        }
        Ok(())
    }
        ...
        if name.contains(':') {
            ...
            if parts.len() == 2 {
                ...
                if let Some(namespace) = self.prefixes.get(prefix) {
                    return Ok(crate::ontology::IRI::new(&format!("{}{}", namespace, local_name)));
                }
            }
        }
...
mod tests {
    ...
    fn test_prefix_parsing() {
        ...
        parser.parse_prefix_declaration("Prefix: ex: <http://example.org/>").unwrap();
        
        assert_eq!(
            parser.prefixes.get("ex").unwrap(),
            "http://example.org/"
        );
    }
