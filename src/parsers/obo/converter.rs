...
use crate::ontology::{
    AnnotationProperty, AnnotationValue, Class, ClassExpression, IRI, Individual, ObjectProperty,
...
impl Obo2Owl {
    ...
    pub fn new() -> Self {
        Self {
            iri_prefix: "http://purl.obolibrary.org/obo/".into(),
        }
    }
        ...
        for stanza in stanzas {
            ...
            match stanza.stanza_type.as_str() {
                "Term" => {
                    ...
                    if let Some(name) = self.get_tag_opt(tags, "name") {
                        ontology.add_axiom(Axiom::AnnotationAssertion(AnnotationAssertionAxiom {
                            ...
                            value: AnnotationValue::Literal(crate::ontology::Literal::new(
                                name.to_string(),
                            )),
                            annotations: vec![],
                        }));
                    ...
                    if let Some(def) = self.get_tag_opt(tags, "def") {
                        ...
                        if !clean.is_empty() {
                            ontology.add_axiom(Axiom::AnnotationAssertion(
                    ...
                    for (_tag, val) in tags.iter().filter(|(t, _)| t == "relationship") {
                        ...
                        let filler = ClassExpression::Class(Class {
                            iri: self.expand(target),
                        });
                        ontology.add_axiom(Axiom::SubClassOf(SubClassOfAxiom {
                            id: 0,
                            subclass: cls.clone(),
                            superclass: ClassExpression::ObjectSomeValuesFrom {
                                property: prop,
                    ...
                    let inter_tags: Vec<&String> = tags
                        .iter()
                        .filter(|(t, _)| t == "intersection_of")
                        .map(|(_, v)| v)
                        .collect();
                    if !inter_tags.is_empty() {
                        ...
                        for val in &inter_tags {
                            ...
                            if let Some(space_pos) = inter_id.find(' ') {
                                ...
                                equivalents.push(ClassExpression::ObjectSomeValuesFrom {
                                    property: prop,
                                    filler: Box::new(filler),
                                });
                            } else {
                                equivalents.push(ClassExpression::Class(Class {
                                    iri: self.expand(inter_id),
                                }));
                            }
                        }
                    ...
                    if self
                        .get_tag_opt(tags, "is_obsolete")
                        .is_some_and(|v| v == "true")
                    {
                        ontology.add_axiom(Axiom::AnnotationAssertion(AnnotationAssertionAxiom {
                            id: 0,
                            property: AnnotationProperty {
                                iri: IRI::new("http://www.w3.org/2002/07/owl#deprecated"),
                            },
                            subject: crate::ontology::AnnotationSubject::IRI(iri.clone()),
                            value: AnnotationValue::Literal(crate::ontology::Literal::new(
                                "true".into(),
                ...
                "Typedef" => {
                    ...
                    for (_tag, val) in tags.iter().filter(|(t, _)| t == "is_a") {
                        ...
                        ontology.add_axiom(Axiom::SubObjectPropertyOf(SubObjectPropertyOfAxiom {
                            ...
                            super_property: ObjectPropertyExpression::ObjectProperty(
                                ObjectProperty {
                                    iri: self.expand(parent_id),
                                },
                            ),
                            annotations: vec![],
                        }));
                    ...
                    if self
                        .get_tag_opt(tags, "is_transitive")
                        .is_some_and(|v| v == "true")
                    {
                    ...
                    if self
                        .get_tag_opt(tags, "is_cyclic")
                        .is_some_and(|v| v == "true")
                    {
                        ontology.add_axiom(Axiom::ReflexiveObjectProperty(
                ...
                "Instance" => {
                    let iri = self.expand(&id);
                    let ind =
                        Individual::Named(crate::ontology::NamedIndividual { iri: iri.clone() });
    ...
    fn expand(&self, id: &str) -> IRI {
        if id.starts_with("http") {
            ...
            IRI::new(&format!("{}{}", self.iri_prefix, id.replace(':', "_")))
        }
    }
}
