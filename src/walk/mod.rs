...
impl<V: OWLObjectVisitor> OntologyWalker<V> {
    ...
    pub fn walk_axiom(&mut self, axiom: &Axiom) {
        ...
        match axiom {
            Axiom::SubClassOf(a) => {
                self.walk_ce(&a.subclass);
                self.walk_ce(&a.superclass);
            }
            ...
            Axiom::DisjointUnion(a) => {
                self.walk_ce(&a.class);
                for c in &a.disjoint_classes {
                    self.walk_ce(c);
                }
            }
            Axiom::SubObjectPropertyOf(a) => {
                self.walk_ope(&a.sub_property);
                self.walk_ope(&a.super_property);
            }
            ...
            Axiom::InverseObjectProperties(a) => {
                self.walk_ope(&a.property1);
                self.walk_ope(&a.property2);
            }
            Axiom::ObjectPropertyDomain(a) => {
                self.walk_ope(&a.property);
                self.walk_ce(&a.domain);
            }
            Axiom::ObjectPropertyRange(a) => {
                self.walk_ope(&a.property);
                self.walk_ce(&a.range);
            }
            ...
            Axiom::SubDataPropertyOf(a) => {
                self.walk_dpe(&a.sub_property);
                self.walk_dpe(&a.super_property);
            }
            ...
            Axiom::DataPropertyDomain(a) => {
                self.walk_dpe(&a.property);
                self.walk_ce(&a.domain);
            }
            Axiom::DataPropertyRange(a) => {
                self.walk_dpe(&a.property);
                self.walk_dr(&a.range);
            }
            ...
            Axiom::ClassAssertion(a) => {
                self.walk_ce(&a.class);
                self.walk_ind(&a.individual);
            }
            Axiom::ObjectPropertyAssertion(a) => {
                self.walk_ope(&a.property);
                self.walk_ind(&a.source);
                self.walk_ind(&a.target);
            }
            Axiom::DataPropertyAssertion(a) => {
                self.walk_dpe(&a.property);
                self.walk_ind(&a.individual);
                self.walk_lit(&a.value);
            }
    ...
    fn walk_ce(&mut self, expr: &ClassExpression) {
        ...
        match expr {
            ClassExpression::Class(cls) => {
                self.visitor.visit_iri(&cls.iri);
            }
    ...
    fn walk_dr(&mut self, dr: &DataRange) {
        ...
        match dr {
            DataRange::Datatype(iri) => {
                self.visitor.visit_iri(iri);
            }
            DataRange::DataIntersectionOf(rs) | DataRange::DataUnionOf(rs) => {
                for r in rs {
                    self.walk_dr(r);
                }
            }
            ...
            DataRange::DatatypeRestriction { datatype, .. } => {
                self.visitor.visit_iri(datatype);
            }
        }
    }
...
impl<'a, V: OWLObjectVisitor> StructureWalker<'a, V> {
    ...
    pub fn new(visitor: V) -> Self {
        Self {
            walker: OntologyWalker::new(visitor),
            current_axiom: None,
        }
    }
