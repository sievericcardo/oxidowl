...
use super::{
    change_tracking::{InvalidationEvent, ChangeTracker},
    delta_computation::{ReasoningDelta, QueryDelta},
...
impl IncrementalCacheManager {
    ...
    pub fn new(
        base_cache: Arc<CacheManager>,
        config: Option<IncrementalCacheConfig>,
    ) -> Self {
    ...
    pub async fn process_invalidation_events(&self, events: Vec<InvalidationEvent>) -> Result<()> {
        ...
        if self.config.enable_statistics {
            self.update_statistics(start_time, total_invalidations).await?;
        }
    ...
    pub async fn apply_reasoning_delta(&self, delta: &ReasoningDelta) -> Result<()> {
        ...
        for (subclass, superclass) in &delta.hierarchy_updates {
            self.invalidate_hierarchy_cache(subclass, superclass).await?;
        }
        ...
        if self.config.enable_statistics {
            ...
            self.update_statistics(start_time, invalidation_count).await?;
        }
            ...
            match event {
                InvalidationEvent::ConceptSatisfiability(classes) => {
                    for class in classes {
                        self.invalidate_concept_cache(&ClassExpression::Class(class.clone())).await?;
    ...
    async fn invalidate_hierarchy_cache(&self, subclass: &Class, superclass: &Class) -> Result<()> {
        ...
        tracing::debug!("Invalidated hierarchy cache for: {} -> {}", subclass.iri, superclass.iri);
    ...
    async fn update_statistics(&self, start_time: Instant, invalidation_count: usize) -> Result<()> {
...
impl SelectiveUpdater {
    ...
    pub async fn update_caches_selectively(&self, delta: &ReasoningDelta) -> Result<()> {
        ...
        tracing::debug!("Performing selective cache updates for delta with {} concepts", 
