...
use crate::{
    ...
    ontology::{
        ...
        ObjectProperty, DataProperty, DataPropertyExpression, ObjectPropertyExpression,
        Ontology,
    },
    query::advanced::{
        conjunctive::ConjunctiveQuery,
        execution::{QueryEngine, AdvancedQueryError},
    },
    reasoning::ReasoningService,
};
use super::{
    change_tracking::{ChangeTracker, TBoxChange, ABoxChange, InvalidationEvent},
    delta_computation::{DeltaComputer, ReasoningDelta, QueryDelta},
    cache_management::{IncrementalCacheManager, ConsistencyReport},
    IncrementalConfig, IncrementalStatistics, ChangeEvent,
};
use std::{
...
impl IncrementalReasoningService {
    ...
    pub async fn add_axiom_incrementally(&self, axiom: Axiom) -> Result<()> {
        ...
        let delta = self.delta_computer.compute_reasoning_delta_for_changes(&[
            TBoxChange::AxiomAdded { axiom, timestamp: start_time }
        ], &[]).await?;
        ...
        self.cache_manager.process_invalidation_events(invalidation_events).await?;
        
        // Update statistics
        self.update_incremental_statistics(start_time, false).await?;
    ...
    pub async fn remove_axiom_incrementally(&self, axiom: &Axiom) -> Result<()> {
        ...
        // Compute and apply reasoning delta
        let delta = self.delta_computer.compute_reasoning_delta_for_changes(&[
            TBoxChange::AxiomRemoved { axiom: axiom.clone(), timestamp: start_time }
        ], &[]).await?;
    ...
    pub async fn add_class_assertion_incrementally(
        ...
        // Compute and apply reasoning delta
        let delta = self.delta_computer.compute_reasoning_delta_for_changes(&[], &[
            ABoxChange::ClassAssertionAdded { individual, class, timestamp: start_time }
        ]).await?;
    ...
    pub async fn execute_query_incrementally(
        ...
        let query_delta = self.delta_computer.compute_query_delta(query, since_timestamp).await?;
        ...
        let result = if query_delta.recommend_full_reexecution || query_delta.is_empty() {
            ...
            QueryResult {
                bindings: results.bindings.into_iter()
                    .map(|binding| {
                        binding.variable_bindings.into_iter()
        ...
        // Update statistics
        self.update_query_statistics(start_time, result.is_incremental).await?;
    ...
    async fn update_incremental_statistics(&self, start_time: Instant, was_full_reasoning: bool) -> Result<()> {
    ...
    async fn update_query_statistics(&self, start_time: Instant, was_incremental: bool) -> Result<()> {
        let elapsed = start_time.elapsed().as_millis() as u64;
...
impl PerformanceReport {
    ...
    pub fn total_memory_saved_bytes(&self) -> usize {
        self.cache_statistics.memory_saved_bytes + 
        self.incremental_statistics.memory_usage_bytes
    }
}