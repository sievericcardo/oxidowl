...
use crate::swrl::{SWRLAtom, SWRLRule, SWRLDArgument, SWRLIArgument};
...
impl RuleValidator {
    ...
    fn normalize_argument_pair(&self, arg1: &SWRLIArgument, arg2: &SWRLIArgument) -> (String, String) {
...
impl GoalChecker {
        ...
        if let Some(ontology) = &engine.ontology {
            ...
            match goal {
                ...
                SWRLAtom::ObjectPropertyAtom { predicate, first_argument, second_argument } => {
                    self.check_object_property_assertion(engine, first_argument, predicate, second_argument)
                }
                SWRLAtom::DataPropertyAtom { predicate, first_argument, second_argument } => {
                    self.check_data_property_assertion(engine, first_argument, predicate, second_argument)
                }
                SWRLAtom::SameIndividualAtom { first_argument, second_argument } => {
    ...
    pub fn find_rules_for_goal(&mut self, engine: &SWRLRuleEngine, goal: &SWRLAtom) -> Result<Vec<u64>> {
            ...
            for axiom in ontology_guard.axioms() {
                if let Axiom::Rule(rule_axiom) = axiom {
                    ...
                    for head_atom in &rule_axiom.rule.head {
                        if self.pattern_matcher.goal_matches_rule_head(goal, head_atom)? {
        ...
        if let Some(ontology) = &engine.ontology {
            ...
            for axiom in ontology_guard.axioms() {
                if let Axiom::ObjectPropertyAssertion(assertion) = axiom {
                    if self.matches_object_property_assertion_axiom(subject, property, object, assertion)? {
                        return Ok(true);
                    }
                }
            }
        ...
        if let Some(ontology) = &engine.ontology {
            ...
            for axiom in ontology_guard.axioms() {
                if let Axiom::DataPropertyAssertion(assertion) = axiom {
                    if self.matches_data_property_assertion_axiom(subject, property, object, assertion)? {
                        return Ok(true);
                    }
                }
            }
    ...
    fn matches_class_assertion_axiom(
        ...
        match individual {
            SWRLIArgument::Individual(ind) => {
                Ok(ind.iri().map(|i| i.as_str()) == assertion.individual.iri().map(|i| i.as_str()) && 
    ...
    fn matches_object_property_assertion_axiom(
        ...
        let subject_match = match subject {
            SWRLIArgument::Individual(ind) => ind.iri().map(|i| i.as_str()) == assertion.source.iri().map(|i| i.as_str()),
            SWRLIArgument::Variable(_) => true, // Variables can match any individual
        };

        let object_match = match object {
            SWRLIArgument::Individual(ind) => ind.iri().map(|i| i.as_str()) == assertion.target.iri().map(|i| i.as_str()),
            SWRLIArgument::Variable(_) => true, // Variables can match any individual
        };
    ...
    fn matches_data_property_assertion_axiom(
        ...
        let object_match = match object {
            SWRLDArgument::Literal(lit) => lit.value == assertion.value.value && lit.datatype == assertion.value.datatype,
    ...
    fn data_properties_match(&self, prop1: &crate::ontology::DataPropertyExpression, prop2: &crate::ontology::DataPropertyExpression) -> bool {
        match (prop1, prop2) {
            (crate::ontology::DataPropertyExpression::DataProperty(p1), 
             crate::ontology::DataPropertyExpression::DataProperty(p2)) => p1.iri == p2.iri,
            _ => false,
        }
    }
}
