...
impl RegexBuiltInRegistry {
    ...
    pub fn new() -> Self {
        ...
        registry.register("http://www.w3.org/2003/11/swrlb#matches", Box::new(MatchesBuiltIn::new(Arc::clone(&cache))));
        registry.register("http://www.w3.org/2003/11/swrlb#replace", Box::new(ReplaceBuiltIn::new(Arc::clone(&cache))));
        
        // Advanced operations
        registry.register("http://www.w3.org/2003/11/swrlb#regexReplace", Box::new(RegexReplaceBuiltIn::new(Arc::clone(&cache))));
        registry.register("http://www.w3.org/2003/11/swrlb#tokenize", Box::new(TokenizeBuiltIn::new(Arc::clone(&cache))));
        registry.register("http://www.w3.org/2003/11/swrlb#split", Box::new(SplitBuiltIn::new(Arc::clone(&cache))));
        
        // Extract operations
        registry.register("http://www.w3.org/2003/11/swrlb#extract", Box::new(ExtractBuiltIn::new(Arc::clone(&cache))));
        registry.register("http://www.w3.org/2003/11/swrlb#extractAll", Box::new(ExtractAllBuiltIn::new(Arc::clone(&cache))));
        
        // Validation
        registry.register("http://www.w3.org/2003/11/swrlb#isValidPattern", Box::new(IsValidPatternBuiltIn));
...
impl SWRLBuiltIn for MatchesBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() < 2 || args.len() > 3 {
            return Err(Error::reasoning("matches expects 2 or 3 arguments (string, pattern, [case_insensitive])"));
        }
...
impl SWRLBuiltIn for ReplaceBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() != 4 {
            return Err(Error::reasoning("replace expects exactly 4 arguments (result, string, pattern, replacement)"));
        }
...
impl SWRLBuiltIn for RegexReplaceBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() < 4 || args.len() > 5 {
            return Err(Error::reasoning("regexReplace expects 4 or 5 arguments (result, string, pattern, replacement, [case_insensitive])"));
        }
...
impl SWRLBuiltIn for TokenizeBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() < 2 || args.len() > 3 {
            return Err(Error::reasoning("tokenize expects 2 or 3 arguments (string, pattern, [case_insensitive])"));
        }
...
impl SWRLBuiltIn for SplitBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() < 2 || args.len() > 3 {
            return Err(Error::reasoning("split expects 2 or 3 arguments (string, delimiter_pattern, [case_insensitive])"));
        }
...
impl SWRLBuiltIn for ExtractBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() < 2 || args.len() > 3 {
            return Err(Error::reasoning("extract expects 2 or 3 arguments (string, pattern, [case_insensitive])"));
        }
...
impl SWRLBuiltIn for ExtractAllBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() < 2 || args.len() > 3 {
            return Err(Error::reasoning("extractAll expects 2 or 3 arguments (string, pattern, [case_insensitive])"));
        }
...
impl SWRLBuiltIn for IsValidPatternBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() != 1 {
            return Err(Error::reasoning("isValidPattern expects exactly 1 argument (pattern)"));
        }
