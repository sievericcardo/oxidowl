...
impl SWRLBuiltIn for ListIntersectionBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() != 3 {
            return Err(Error::reasoning("ListIntersection expects exactly 3 arguments (result, list1, list2)"));
        }

        match (&args[0], &args[1], &args[2]) {
            (SWRLValue::String(result), SWRLValue::String(list1), SWRLValue::String(list2)) => {
                let items1: Vec<&str> = if list1.is_empty() { Vec::new() } else { list1.split(',').map(|s| s.trim()).collect() };
                let items2: Vec<&str> = if list2.is_empty() { Vec::new() } else { list2.split(',').map(|s| s.trim()).collect() };
                
                let intersection: Vec<&str> = items1.into_iter()
            ...
            _ => Err(Error::reasoning("ListIntersection requires string arguments")),
        }
    }
...
impl SWRLBuiltIn for ListSubtractionBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() != 3 {
            return Err(Error::reasoning("ListSubtraction expects exactly 3 arguments (result, list1, list2)"));
        }
        ...
        match (&args[0], &args[1], &args[2]) {
            (SWRLValue::String(result), SWRLValue::String(list1), SWRLValue::String(list2)) => {
                ...
                let difference: Vec<&str> = items1.into_iter()
                    .filter(|item| !items2.contains(item))
                    .collect();
            ...
            _ => Err(Error::reasoning("ListSubtraction requires string arguments")),
        }
    }
...
impl SWRLBuiltIn for FirstBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() != 2 {
            return Err(Error::reasoning("First expects exactly 2 arguments (result, list)"));
        }
...
impl SWRLBuiltIn for RestBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() != 2 {
            return Err(Error::reasoning("Rest expects exactly 2 arguments (result, list)"));
        }
...
impl SWRLBuiltIn for SublistBuiltIn {
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        if args.len() != 4 {
            return Err(Error::reasoning("Sublist expects exactly 4 arguments (result, list, start, length)"));
        }

        match (&args[0], &args[1], &args[2], &args[3]) {
            (SWRLValue::String(result), SWRLValue::String(list), SWRLValue::Integer(start), SWRLValue::Integer(length)) => {
                if list.is_empty() {
                    return Ok(SWRLValue::Boolean(result.is_empty()));
                }
            ...
            _ => Err(Error::reasoning("Sublist requires string list and integer indices")),
        }
    }
    ...
    fn execute(&self, args: &[SWRLValue]) -> Result<SWRLValue> {
        ...
        match &args[0] {
            SWRLValue::String(list) => {
                Ok(SWRLValue::Boolean(list.is_empty() || list.trim().is_empty()))
            }
