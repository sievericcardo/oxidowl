...
impl ResultAggregator {
    ...
    pub async fn start_aggregation(&self, query_id: Uuid, expected_partitions: usize) 
        ...
        info!("Started aggregation session for query {} expecting {} partitions", 
    ...
    pub async fn add_partial_result(&self, partial_result: PartialResult) -> Result<()> {
        ...
        let should_aggregate = {
            ...
            if let Some(session) = active_sessions.get_mut(&query_id) {
                session.partial_results.insert(partial_result.partition_id, partial_result);
    ...
    async fn complete_aggregation(&self, query_id: Uuid) -> Result<()> {
        ...
        info!("Completing aggregation for query {} with {} partial results", 
              query_id, session.partial_results.len());
    ...
    pub async fn aggregate_results(&self, partial_results: Vec<PartialResult>) -> Result<AggregatedResult> {
        ...
        if self.config.enable_consistency_checking {
            ...
            let (consistent_bindings, resolved_count) = consistency_checker
                .resolve_inconsistencies(all_bindings).await?;
        ...
        let quality = {
            ...
            quality_assessor.assess_quality(&partial_results, &all_bindings).await?
        };
        ...
        info!("Aggregation completed: {} final bindings, quality score: {:.2}", 
              aggregated_result.bindings.len(), aggregated_result.quality.overall_quality);
...
impl DuplicateDetector {
    ...
    pub async fn remove_duplicates(&self, bindings: Vec<QueryBinding>) -> Result<Vec<QueryBinding>> {
...
impl ConsistencyChecker {
    ...
    pub async fn resolve_inconsistencies(&self, bindings: Vec<QueryBinding>) 
...
impl QualityAssessor {
    ...
    pub async fn assess_quality(&self, partial_results: &[PartialResult], 
                               bindings: &[QueryBinding]) -> Result<ResultQuality> {
        
        // Calculate completeness based on partition success rate
        let successful_partitions = partial_results.iter()
        ...
        if completeness < 0.9 {
            issues.push(QualityIssue {
                ...
                description: format!("Only {:.1}% of partitions completed successfully", 
                                   completeness * 100.0),
                affected_partitions: partial_results.iter()
                    .filter(|r| r.status != PartialResultStatus::Complete)
...
mod tests {
    use super::*;
    use crate::query::{Variable, Term, Substitution};
    ...
    async fn test_duplicate_detection() {
        ...
        let binding1 = QueryBinding {
            substitution: {
                ...
                sub.insert(Variable("x".to_string()), Term::Individual("John".to_string()));
                sub
            },
        };
        ...
        let binding3 = QueryBinding {
            substitution: {
                let mut sub = Substitution::new();
                sub.insert(Variable("x".to_string()), Term::Individual("Jane".to_string()));
                sub
            },
        };
