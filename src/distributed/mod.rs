...
pub use cluster::{ClusterManager, NodeInfo, ClusterState, NodeHealth};
pub use query_distribution::{QueryDistributor, DistributedQuery, QueryPartition};
pub use result_aggregation::{ResultAggregator, PartialResult, AggregatedResult};
pub use fault_tolerance::{FaultTolerance, FailureDetector, RecoveryStrategy};
pub use load_balancing::{LoadBalancer, NodeLoad, WorkloadDistribution};
pub use coordination::{ClusterCoordinator, DistributedLock, ConsensusProtocol};
...
pub enum DistributionStrategy {
    ...
    Hybrid { strategies: Vec<DistributionStrategy> },
}
...
impl DistributedQueryProcessor {
    ...
    pub async fn new(config: DistributedConfig) -> Result<Self> {
        ...
        let result_aggregator = Arc::new(RwLock::new(
            ResultAggregator::new().await?
        ));
    ...
    pub async fn execute_distributed_query(&self, query: ConjunctiveQuery) -> Result<AggregatedResult> {
        info!("Executing distributed query with {} atoms", query.body_atoms.len());
        ...
        let partial_results = self.execute_query_partitions(distributed_query.partitions).await?;
    ...
    async fn execute_query_partitions(&self, partitions: Vec<QueryPartition>) -> Result<Vec<PartialResult>> {
...
mod tests {
    ...
    fn test_node_capabilities() {
        ...
        assert!(capabilities.reasoning_features.contains(&"SROIQ".to_string()));
