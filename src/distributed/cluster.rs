...
use crate::prelude::*;
use crate::distributed::{NodeId, NodeAddress, DistributedError};
use std::collections::HashMap;
use std::sync::Arc;
use tokio::sync::{RwLock, mpsc, Mutex};
use tokio::time::{Duration, Instant, sleep, timeout, interval};
use uuid::Uuid;
...
impl ClusterManager {
    ...
    async fn process_cluster_event(state: &Arc<RwLock<ClusterState>>, event: ClusterEvent) -> Result<()> {
    ...
    pub async fn add_node(&mut self, node_info: NodeInfo) -> Result<()> {
        ...
        self.event_sender.send(event)
            .map_err(|e| DistributedError::Cluster(format!("Failed to send node join event: {}", e)))?;
        Ok(())
    }
    ...
    pub async fn remove_node(&mut self, node_id: NodeId) -> Result<()> {
        ...
        self.event_sender.send(event)
            .map_err(|e| DistributedError::Cluster(format!("Failed to send node leave event: {}", e)))?;
        Ok(())
    }
...
impl HealthMonitor {
    ...
    /// Start health monitoring
    pub async fn start(&mut self, event_sender: mpsc::UnboundedSender<ClusterEvent>, 
                      state: Arc<RwLock<ClusterState>>) -> Result<()> {
    ...
    async fn perform_health_checks(state: &Arc<RwLock<ClusterState>>, 
                                  event_sender: &mpsc::UnboundedSender<ClusterEvent>) -> Result<()> {
        let nodes = {
...
impl DiscoveryService {
    ...
    async fn discover_nodes(config: &crate::distributed::ClusterConfig, 
                           event_sender: &mpsc::UnboundedSender<ClusterEvent>) -> Result<()> {
    ...
    async fn discover_multicast(_address: &str, _port: u16, 
                               _event_sender: &mpsc::UnboundedSender<ClusterEvent>) -> Result<()> {
    ...
    async fn discover_static(nodes: &[NodeAddress], 
                           event_sender: &mpsc::UnboundedSender<ClusterEvent>) -> Result<()> {
    ...
    async fn discover_external(_service_url: &str, 
                              _event_sender: &mpsc::UnboundedSender<ClusterEvent>) -> Result<()> {
    ...
    async fn discover_consul(_consul_address: &str, 
                            _event_sender: &mpsc::UnboundedSender<ClusterEvent>) -> Result<()> {
    ...
    fn test_cluster_health_calculation() {
        ...
        for i in 0..5 {
            let node_info = NodeInfo {
                ...
                capabilities: crate::distributed::NodeCapabilities::default(),
                status: if i < 4 { NodeStatus::Active } else { NodeStatus::Failed },
                metadata: HashMap::new(),
                last_seen: Instant::now(),
                version: "1.0.0".to_string(),
            };
