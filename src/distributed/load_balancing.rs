...
use crate::prelude::*;
use super::{NodeId, LoadBalancingConfig, LoadBalancingAlgorithm};
...
impl LoadBalancer {
    ...
    pub async fn start(&mut self) -> Result<()> {
        info!("Load balancer started with algorithm: {:?}", self.config.algorithm);
        Ok(())
    }
            ...
            LoadBalancingAlgorithm::MemoryBased => {
                ...
                available_nodes
                    ...
                    .min_by(|a, b| {
                        ...
                        mem_a.partial_cmp(&mem_b).unwrap_or(std::cmp::Ordering::Equal)
                    })
                    .copied()
            }
