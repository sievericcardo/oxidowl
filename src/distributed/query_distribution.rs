...
impl QueryDistributor {
    ...
    pub async fn distribute_query(&self, query: &ConjunctiveQuery, 
                                 cluster_manager: &ClusterManager) -> Result<DistributedQuery> {
        let query_id = Uuid::new_v4();
        
        info!("Distributing query {} with {} atoms", query_id, query.body_atoms.len());
        ...
        if available_nodes.is_empty() {
            return Err(DistributedError::Distribution("No active nodes available".to_string()).into());
        }
        
        // Create query partitions based on strategy
        let partitions = self.create_partitions(query, &metadata, &available_nodes).await?;
        ...
        info!("Query {} distributed into {} partitions", query_id, distributed_query.partitions.len());
        Ok(distributed_query)
    }
    
    /// Create query partitions based on the distribution strategy
    async fn create_partitions(&self, query: &ConjunctiveQuery, metadata: &QueryMetadata, 
                              available_nodes: &[NodeInfo]) -> Result<Vec<QueryPartition>> {
        match &self.config.strategy {
            crate::distributed::DistributionStrategy::ConceptBased => {
                self.partition_by_concepts(query, metadata, available_nodes).await
            }
            crate::distributed::DistributionStrategy::ComplexityBased => {
                self.partition_by_complexity(query, metadata, available_nodes).await
            }
            crate::distributed::DistributionStrategy::RoundRobin => {
                self.partition_round_robin(query, metadata, available_nodes).await
            }
            crate::distributed::DistributionStrategy::LoadAware => {
                self.partition_load_aware(query, metadata, available_nodes).await
            }
            crate::distributed::DistributionStrategy::Hybrid { strategies } => {
                self.partition_hybrid(query, metadata, available_nodes, strategies).await
            }
        }
    }
    
    /// Partition query by ontology concepts
    async fn partition_by_concepts(&self, query: &ConjunctiveQuery, metadata: &QueryMetadata, 
                                  available_nodes: &[NodeInfo]) -> Result<Vec<QueryPartition>> {
        ...
        for atom in &query.body_atoms {
            if let Some(concept) = self.extract_concept_from_atom(atom) {
                concept_groups.entry(concept).or_insert_with(Vec::new).push(atom);
            }
        }
        ...
        for (concept, atoms) in concept_groups {
            ...
            let estimated_cost = {
                ...
                cost_estimator.estimate_cost(&partition_query, metadata).await?
            };
    ...
    async fn partition_by_complexity(&self, query: &ConjunctiveQuery, metadata: &QueryMetadata, 
                                    available_nodes: &[NodeInfo]) -> Result<Vec<QueryPartition>> {
        let mut partitions = Vec::new();
        let query_id = Uuid::new_v4();
        ...
        let chunk_size = (query.body_atoms.len() + available_nodes.len() - 1) / available_nodes.len();
    ...
    async fn partition_round_robin(&self, query: &ConjunctiveQuery, metadata: &QueryMetadata, 
                                  available_nodes: &[NodeInfo]) -> Result<Vec<QueryPartition>> {
        let mut partitions = Vec::new();
        let query_id = Uuid::new_v4();
    ...
    async fn partition_load_aware(&self, query: &ConjunctiveQuery, metadata: &QueryMetadata, 
                                 available_nodes: &[NodeInfo]) -> Result<Vec<QueryPartition>> {
    ...
    async fn partition_hybrid(&self, query: &ConjunctiveQuery, metadata: &QueryMetadata, 
                             available_nodes: &[NodeInfo], _strategies: &[crate::distributed::DistributionStrategy]) -> Result<Vec<QueryPartition>> {
    ...
    /// Compute dependencies between partitions
    async fn compute_partition_dependencies(&self, partitions: &mut [QueryPartition]) -> Result<()> {
    ...
    pub async fn schedule_partitions(&self, partitions: &[QueryPartition]) -> Result<Vec<Uuid>> {
        ...
        while !remaining.is_empty() {
            ...
            for partition in partitions {
                ...
                let deps_satisfied = partition.dependencies.iter()
            ...
            if !made_progress {
                return Err(DistributedError::Distribution("Circular dependency detected".to_string()).into());
            }
        }
...
impl CostEstimator {
    ...
    pub async fn estimate_cost(&self, query: &ConjunctiveQuery, 
...
mod tests {
    use super::*;
    use crate::query::{Variable, Term, Atom};
