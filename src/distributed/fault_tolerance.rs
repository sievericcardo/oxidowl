...
use crate::distributed::cluster::{ClusterManager, NodeInfo, NodeHealth, HealthStatus};
...
impl FaultTolerance {
        ...
        tokio::spawn(async move {
            ...
            while let Some(event) = receiver.recv().await {
                if let Err(e) = Self::process_fault_tolerance_event(
                    event, &recovery_sessions, &circuit_breakers).await {
    ...
    async fn process_fault_tolerance_event(
        ...
        match event {
            ...
            FaultToleranceEvent::RecoveryCompleted(session_id, result) => {
                info!("Recovery session completed: {} - success: {}", 
    ...
    pub async fn execute_with_retry(&self, partition: &QueryPartition, 
        ...
        loop {
            ...
            if !should_attempt {
                return Err(DistributedError::FaultTolerance(
                    format!("Circuit breaker open for node {}", partition.assigned_node)
                ).into());
            }
            ...
            match self.execute_partition(partition, cluster_manager).await {
                ...
                Err(e) => {
                    ...
                    if attempts > 1 {
                        if let Ok(new_node) = self.find_alternative_node(&partition.assigned_node, cluster_manager).await {
                            // Create new partition with different assignment
                            let mut new_partition = partition.clone();
                            new_partition.assigned_node = new_node;
                            
                            match self.execute_partition(&new_partition, cluster_manager).await {
    ...
    async fn execute_partition(&self, partition: &QueryPartition, 
        ...
        match timeout(execution_timeout, self.do_execute_partition(partition)).await {
            ...
            Err(_) => {
                ...
                Err(DistributedError::FaultTolerance(
                    format!("Query execution timeout on node {}", partition.assigned_node)
                ).into())
            }
        }
    }
    ...
    async fn do_execute_partition(&self, partition: &QueryPartition) 
                                -> Result<crate::distributed::result_aggregation::PartialResult> {
    ...
    async fn find_alternative_node(&self, failed_node: &NodeId, 
                                  cluster_manager: &Arc<RwLock<ClusterManager>>) -> Result<NodeId> {
    ...
    pub async fn create_checkpoint(&self, description: String) -> Result<Uuid> {
        ...
        
        {
            ...
            checkpoint_manager.create_checkpoint(checkpoint_id, description.clone()).await?;
        }
...
impl FailureDetector {
    ...
    pub async fn start(&mut self, event_sender: mpsc::UnboundedSender<FaultToleranceEvent>) -> Result<()> {
...
impl RecoveryManager {
    ...
    pub async fn start(&mut self, _event_sender: mpsc::UnboundedSender<FaultToleranceEvent>) -> Result<()> {
        let (shutdown_tx, mut shutdown_rx) = mpsc::channel::<()>(1);
        self.shutdown_tx = Some(shutdown_tx);
...
mod tests {
    ...
    async fn test_checkpoint_manager() {
        ...
        let result = manager.create_checkpoint(checkpoint_id, "Test checkpoint".to_string()).await;
        assert!(result.is_ok());
